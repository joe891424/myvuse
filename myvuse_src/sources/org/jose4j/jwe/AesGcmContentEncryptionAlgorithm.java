...
public class AesGcmContentEncryptionAlgorithm extends AlgorithmInfo implements ContentEncryptionAlgorithm {
    ...
    public ContentEncryptionParts encrypt(byte[] bArr, byte[] bArr2, byte[] bArr3, Headers headers, byte[] bArr4, ProviderContext providerContext) throws JoseException {
        return encrypt(bArr, bArr2, bArr3, InitializationVectorHelp.m5642iv(12, bArr4, providerContext.getSecureRandom()), ContentEncryptionHelp.getCipherProvider(headers, providerContext));
    }

    public ContentEncryptionParts encrypt(byte[] bArr, byte[] bArr2, byte[] bArr3, byte[] bArr4, String str) throws JoseException {
        SimpleAeadCipher.CipherOutput cipherOutputEncrypt = this.simpleAeadCipher.encrypt(new AesKey(bArr3), bArr4, bArr, bArr2, str);
        return new ContentEncryptionParts(bArr4, cipherOutputEncrypt.getCiphertext(), cipherOutputEncrypt.getTag());
    }
    ...
    public static class Aes192Gcm extends AesGcmContentEncryptionAlgorithm {
        public Aes192Gcm() {
            super(ContentEncryptionAlgorithmIdentifiers.AES_192_GCM, JfifUtil.MARKER_SOFn);
        }
    }
