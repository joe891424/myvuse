...
public class SimpleJwkFilter {
    ...
    public List<JsonWebKey> filter(Collection<JsonWebKey> collection) {
        ...
        for (JsonWebKey jsonWebKey : collection) {
            boolean zIsMatch = isMatch(this.kid, jsonWebKey.getKeyId()) & isMatch(this.kty, jsonWebKey.getKeyType()) & isMatch(this.use, jsonWebKey.getUse()) & isMatch(this.alg, jsonWebKey.getAlgorithm());
            String[] thumbs = getThumbs(jsonWebKey, this.allowThumbsFallbackDeriveFromX5c);
            boolean zIsMatch2 = zIsMatch & isMatch(this.x5t, thumbs[0]) & isMatch(this.x5tS256, thumbs[1]) & isMatch(this.crv, getCrv(jsonWebKey));
            KeyOpsCriteria keyOpsCriteria = this.keyOps;
            if (zIsMatch2 & (keyOpsCriteria == null || keyOpsCriteria.meetsCriteria(jsonWebKey.getKeyOps()))) {
