...
abstract class KeyPairUtil {
    ...
    public PublicKey fromPemEncoded(String str) throws InvalidKeySpecException, JoseException {
        return getKeyFactory().generatePublic(new X509EncodedKeySpec(SimplePEMEncoder.decode(str.substring(str.indexOf(BEGIN_PUBLIC_KEY) + BEGIN_PUBLIC_KEY.length(), str.indexOf(END_PUBLIC_KEY)).trim())));
    }

    public static String pemEncode(PublicKey publicKey) {
        return "-----BEGIN PUBLIC KEY-----\r\n" + SimplePEMEncoder.encode(publicKey.getEncoded()) + END_PUBLIC_KEY;
    }
