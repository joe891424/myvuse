...
public final class ShortcutBadger {
    ...
    public static boolean applyCount(Context context, int i) {
        try {
            ...
            LogInstrumentation.m2727d(LOG_TAG, "Unable to execute badge", e);
            return false;
        }
    }
        ...
        if (sIsBadgeCounterSupported == null) {
            synchronized (sCounterSupportedLock) {
                if (sIsBadgeCounterSupported == null) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        try {
                            LogInstrumentation.m2730i(LOG_TAG, "Checking if platform supports badge counters, attempt " + String.format("%d/%d.", Integer.valueOf(i + 1), 3));
                            if (initBadger(context)) {
                                sShortcutBadger.executeBadge(context, sComponentName, 0);
                                sIsBadgeCounterSupported = true;
                                LogInstrumentation.m2730i(LOG_TAG, "Badge counter is supported in this platform.");
                                break;
                            }
                    ...
                    if (sIsBadgeCounterSupported == null) {
                        LogInstrumentation.m2734w(LOG_TAG, "Badge counter seems not supported for this platform: " + message);
    ...
    private static boolean initBadger(Context context) {
        ...
        if (launchIntentForPackage == null) {
            LogInstrumentation.m2728e(LOG_TAG, "Unable to find launch intent for package " + context.getPackageName());
            return false;
        }
