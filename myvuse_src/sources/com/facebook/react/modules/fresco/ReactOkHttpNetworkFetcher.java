...
class ReactOkHttpNetworkFetcher extends OkHttpNetworkFetcher {
    ...
    public void fetch(OkHttpNetworkFetcher.OkHttpNetworkFetchState okHttpNetworkFetchState, NetworkFetcher.Callback callback) {
        ...
        Request.Builder builder = new Request.Builder().cacheControl(new CacheControl.Builder().noStore().build()).url(uri.toString()).headers(Headers.m5626of(headers)).get();
        fetchWithRequest(okHttpNetworkFetchState, callback, !(builder instanceof Request.Builder) ? builder.build() : OkHttp3Instrumentation.build(builder));
    }
}
