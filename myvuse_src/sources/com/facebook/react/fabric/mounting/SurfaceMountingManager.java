...
public class SurfaceMountingManager {
    ...
    private static void logViewHierarchy(ViewGroup viewGroup, boolean z) {
        ...
        FLog.m1277e(TAG, "  <ViewGroup tag=" + id + " class=" + viewGroup.getClass().toString() + ">");
        for (int i = 0; i < viewGroup.getChildCount(); i++) {
            FLog.m1277e(TAG, "     <View idx=" + i + " tag=" + viewGroup.getChildAt(i).getId() + " class=" + viewGroup.getChildAt(i).getClass().toString() + ">");
        }
        String str = TAG;
        FLog.m1277e(str, "  </ViewGroup tag=" + id + ">");
        if (z) {
            FLog.m1277e(str, "Displaying Ancestors:");
            for (ViewParent parent = viewGroup.getParent(); parent != null; parent = parent.getParent()) {
                ViewGroup viewGroup2 = parent instanceof ViewGroup ? (ViewGroup) parent : null;
                FLog.m1277e(TAG, "<ViewParent tag=" + (viewGroup2 == null ? -1 : viewGroup2.getId()) + " class=" + parent.getClass().toString() + ">");
            }
        }
    }
    ...
    public /* synthetic */ void lambda$addRootView$0(View view) {
        ...
        if (view.getId() == this.mSurfaceId) {
            ...
            FLog.m1279e(TAG, "Trying to add RootTag to RootView that already has a tag: existing tag: [%d] new tag: [%d]", Integer.valueOf(view.getId()), Integer.valueOf(this.mSurfaceId));
    ...
    public void stopSurface() {
        FLog.m1277e(TAG, "Stopping surface [" + this.mSurfaceId + "]");
        if (isStopped()) {
            return;
        }
    ...
    public /* synthetic */ void lambda$stopSurface$1() {
        if (ReactFeatureFlags.fixStoppedSurfaceTagSetLeak) {
            ...
            Iterator<ViewState> it2 = this.mTagToViewState.values().iterator();
            while (it2.hasNext()) {
                onViewStateDeleted(it2.next());
            }
        ...
        FLog.m1277e(TAG, "Surface [" + this.mSurfaceId + "] was stopped on SurfaceMountingManager.");
    }
    ...
    public void removeViewAt(int i, int i2, int i3) {
        ...
        if (id != i) {
            ...
            if (i4 == -1) {
                FLog.m1277e(TAG, "removeViewAt: [" + i + "] -> [" + i2 + "] @" + i3 + ": view already removed from parent! Children in parent: " + childCount);
                return;
            } else {
    ...
    public void removeDeleteTreeAt(int i, int i2, int i3) {
        ...
        if (id != i) {
            ...
            if (i4 == -1) {
                FLog.m1277e(TAG, "removeDeleteTreeAt: [" + i + "] -> [" + i2 + "] @" + i3 + ": view already removed from parent! Children in parent: " + childCount);
                return;
            } else {
                logViewHierarchy(viewGroup, true);
    ...
    public void printSurfaceState() {
        FLog.m1279e(TAG, "Views created for surface {%d}:", Integer.valueOf(getSurfaceId()));
        for (ViewState viewState : this.mTagToViewState.values()) {
            ...
            FLog.m1279e(TAG, "<%s id=%d parentTag=%s isRoot=%b />", name, Integer.valueOf(viewState.mReactTag), numValueOf, Boolean.valueOf(viewState.mIsRoot));
        }
    }
    ...
    private class RemoveDeleteTreeUIFrameCallback extends GuardedFrameCallback {
        ...
        private boolean haveExceededNonBatchedFrameTime(long j) {
            return 16 - ((System.nanoTime() - j) / Constants.Network.MAX_PAYLOAD_SIZE) < MAX_TIME_IN_FRAME;
        }
