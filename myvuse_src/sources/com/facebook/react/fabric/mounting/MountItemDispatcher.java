...
public class MountItemDispatcher {
    ...
    private boolean dispatchMountItems() {
        ...
        if (andResetPreMountItems != null) {
            ...
            Iterator<MountItem> it2 = andResetPreMountItems.iterator();
            while (it2.hasNext()) {
                executeOrEnqueue(it2.next());
            }
            Systrace.endSection(0L);
        }
        if (andResetMountItems != null) {
            ...
            Iterator<MountItem> it3 = andResetMountItems.iterator();
            while (it3.hasNext()) {
    ...
    private void executeOrEnqueue(MountItem mountItem) {
        if (this.mMountingManager.isWaitingForViewAttach(mountItem.getSurfaceId())) {
            if (FabricUIManager.ENABLE_FABRIC_LOGS) {
                FLog.m1279e(TAG, "executeOrEnqueue: Item execution delayed, surface %s is not ready yet", Integer.valueOf(mountItem.getSurfaceId()));
            }
    ...
    private static boolean haveExceededNonBatchedFrameTime(long j) {
        return 16 - ((System.nanoTime() - j) / Constants.Network.MAX_PAYLOAD_SIZE) < 8;
    }
