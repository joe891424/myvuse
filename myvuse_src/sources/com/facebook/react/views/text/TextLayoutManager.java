...
public class TextLayoutManager {
    ...
    private static void buildSpannableFromFragment(Context context, ReadableArray readableArray, SpannableStringBuilder spannableStringBuilder, List<SetSpanOperation> list) {
        ...
        for (int size = readableArray.size(); i2 < size; size = i) {
            ...
            spannableStringBuilder.append((CharSequence) TextTransform.apply(map.getString(TypedValues.Custom.S_STRING), textAttributePropsFromReadableMap.mTextTransform));
            ...
            if (map.hasKey(ViewProps.IS_ATTACHMENT) && map.getBoolean(ViewProps.IS_ATTACHMENT)) {
                list.add(new SetSpanOperation(spannableStringBuilder.length() - INLINE_VIEW_PLACEHOLDER.length(), spannableStringBuilder.length(), new TextInlineViewPlaceholderSpan(i3, (int) PixelUtil.toPixelFromSP(map.getDouble("width")), (int) PixelUtil.toPixelFromSP(map.getDouble("height")))));
            } else {
                if (length2 >= length) {
