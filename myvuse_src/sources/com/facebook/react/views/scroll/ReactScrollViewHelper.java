...
public class ReactScrollViewHelper {
    ...
    private static <T extends ViewGroup & HasScrollEventThrottle> void emitScrollEvent(T t, ScrollEventType scrollEventType, float f, float f2) {
        ...
        if (t2.getScrollEventThrottle() < Math.max(17L, jCurrentTimeMillis - t2.getLastScrollDispatchTime()) && (childAt = t.getChildAt(0)) != null) {
            Iterator<ScrollListener> it2 = sScrollListeners.iterator();
            while (it2.hasNext()) {
                it2.next().onScroll(t, scrollEventType, f, f2);
            }
    ...
    public static int parseOverScrollMode(String str) {
        if (str != null && !str.equals("auto")) {
            ...
            FLog.m1317w(ReactConstants.TAG, "wrong overScrollMode: " + str);
        }
        return 1;
    }
    ...
    public static int parseSnapToAlignment(String str) {
        ...
        FLog.m1317w(ReactConstants.TAG, "wrong snap alignment value: " + str);
        return 0;
    }
    ...
    public static <T extends ViewGroup & HasStateWrapper & HasScrollState & HasFlingAnimator> void smoothScrollTo(T t, int i, int i2) {
        if (DEBUG_MODE) {
            FLog.m1292i(TAG, "smoothScrollTo[%d] x %d y %d", Integer.valueOf(t.getId()), Integer.valueOf(i), Integer.valueOf(i2));
        }
        T t2 = t;
    ...
    public static <T extends ViewGroup & HasStateWrapper & HasScrollState & HasFlingAnimator> void updateFabricScrollState(T t, int i, int i2) {
        if (DEBUG_MODE) {
            FLog.m1292i(TAG, "updateFabricScrollState[%d] scrollX %d scrollY %d", Integer.valueOf(t.getId()), Integer.valueOf(i), Integer.valueOf(i2));
        }
    ...
    public static <T extends ViewGroup & HasScrollState & HasStateWrapper & HasFlingAnimator> void forceUpdateState(T t) {
        ...
        if (DEBUG_MODE) {
            FLog.m1293i(TAG, "updateFabricScrollState[%d] scrollX %d scrollY %d fabricScrollX", Integer.valueOf(t.getId()), Integer.valueOf(i2), Integer.valueOf(i3), Integer.valueOf(i));
        }
