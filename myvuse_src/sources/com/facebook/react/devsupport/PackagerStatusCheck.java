...
public class PackagerStatusCheck {
    ...
    public void run(String str, final PackagerStatusCallback packagerStatusCallback) {
        Request.Builder builderUrl = new Request.Builder().url(createPackagerStatusURL(str));
        ...
        FirebasePerfOkHttpClient.enqueue(!(okHttpClient instanceof OkHttpClient) ? okHttpClient.newCall(requestBuild) : OkHttp3Instrumentation.newCall(okHttpClient, requestBuild), new Callback() { // from class: com.facebook.react.devsupport.PackagerStatusCheck.1
            @Override // okhttp3.Callback
            public void onFailure(Call call, IOException iOException) {
                FLog.m1317w(ReactConstants.TAG, "The packager does not seem to be running as we got an IOException requesting its status: " + iOException.getMessage());
            ...
            public void onResponse(Call call, Response response) throws IOException {
                if (!response.isSuccessful()) {
                    FLog.m1277e(ReactConstants.TAG, "Got non-success http code from packager when requesting status: " + response.code());
                    packagerStatusCallback.onPackagerStatusFetched(false);
                    return;
                }
                ResponseBody responseBodyBody = response.body();
                if (responseBodyBody == null) {
                    FLog.m1277e(ReactConstants.TAG, "Got null body response from packager when requesting status");
                    packagerStatusCallback.onPackagerStatusFetched(false);
                    return;
                }
                String strString = responseBodyBody.string();
                if (!PackagerStatusCheck.PACKAGER_OK_STATUS.equals(strString)) {
                    FLog.m1277e(ReactConstants.TAG, "Got unexpected response from packager when requesting status: " + strString);
                    packagerStatusCallback.onPackagerStatusFetched(false);
                } else {
