...
class FrameBasedAnimationDriver extends AnimationDriver {
    ...
    public void runAnimationStep(long j) {
        ...
        int iRound = (int) Math.round(((j - this.mStartFrameTimeNanos) / Constants.Network.MAX_PAYLOAD_SIZE) / FRAME_TIME_MILLIS);
        if (iRound < 0) {
            FLog.m1317w(ReactConstants.TAG, "Calculated frame index should never be lower than 0. Called with frameTimeNanos " + j + " and mStartFrameTimeNanos " + this.mStartFrameTimeNanos);
            return;
        }
