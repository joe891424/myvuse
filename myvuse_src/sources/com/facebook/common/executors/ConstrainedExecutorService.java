...
public class ConstrainedExecutorService extends AbstractExecutorService {
    ...
    public void execute(Runnable runnable) {
        ...
        if (size > i && this.mMaxQueueSize.compareAndSet(i, size)) {
            FLog.m1299v(TAG, "%s: max pending work in queue = %d", this.mName, Integer.valueOf(size));
        }
        startWorkerIfNeeded();
    }
    ...
    public void startWorkerIfNeeded() {
        ...
        while (i < this.mMaxConcurrency) {
            ...
            if (this.mPendingWorkers.compareAndSet(i, i2)) {
                FLog.m1300v(TAG, "%s: starting worker %d of %d", this.mName, Integer.valueOf(i2), Integer.valueOf(this.mMaxConcurrency));
                this.mExecutor.execute(this.mTaskRunner);
                return;
            } else {
                FLog.m1298v(TAG, "%s: race in startWorkerIfNeeded; retrying", this.mName);
    ...
    private class Worker implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (runnable == null) {
                    FLog.m1298v((Class<?>) ConstrainedExecutorService.TAG, "%s: Worker has nothing to run", ConstrainedExecutorService.this.mName);
                } else {
                    runnable.run();
                }
                ...
                if (!ConstrainedExecutorService.this.mWorkQueue.isEmpty()) {
                    ...
                    FLog.m1299v((Class<?>) ConstrainedExecutorService.TAG, "%s: worker finished; %d workers left", ConstrainedExecutorService.this.mName, Integer.valueOf(iDecrementAndGet));
                }
            } catch (Throwable th) {
                ...
                if (!ConstrainedExecutorService.this.mWorkQueue.isEmpty()) {
                    ...
                    FLog.m1299v((Class<?>) ConstrainedExecutorService.TAG, "%s: worker finished; %d workers left", ConstrainedExecutorService.this.mName, Integer.valueOf(iDecrementAndGet2));
                }
                throw th;
            }
        }
