...
public final class BufferedDiskCache {
    ...
    private final Task<Boolean> containsAsync(final CacheKey key) {
        try {
            ...
            FLog.m1316w(TAG, e, "Failed to schedule disk-cache read for %s", key.getUriString());
    ...
    public final Task<Void> probe(final CacheKey key) {
        ...
        try {
            ...
            FLog.m1316w(TAG, e, "Failed to schedule disk-cache probe for %s", key.getUriString());
    ...
    private final boolean checkInStagingAreaAndFileCache(CacheKey key) {
        ...
        if (encodedImage != null) {
            encodedImage.close();
            FLog.m1298v(TAG, "Found image for %s in staging area", key.getUriString());
            this.imageCacheStatsTracker.onStagingAreaHit(key);
            return true;
        }
        FLog.m1298v(TAG, "Did not find image for %s in staging area", key.getUriString());
        ...
        try {
            ...
            if (encodedImage != null) {
                ...
                try {
                    ...
                    CloseableReference closeableReferenceM1331of = CloseableReference.m1331of(fromDiskCache);
                    Intrinsics.checkNotNullExpressionValue(closeableReferenceM1331of, "of(buffer)");
                    try {
                        encodedImage = new EncodedImage((CloseableReference<PooledByteBuffer>) closeableReferenceM1331of);
                    } finally {
                        CloseableReference.closeSafely((CloseableReference<?>) closeableReferenceM1331of);
                    }
                } catch (Exception unused) {
                    return null;
                }
            ...
            FLog.m1297v(TAG, "Host thread was interrupted, decreasing reference count");
            encodedImage.close();
    ...
    public final void put(final CacheKey key, EncodedImage encodedImage) {
        ...
        try {
            ...
            try {
                ...
                FLog.m1316w(TAG, e, "Failed to schedule disk-cache write for %s", key.getUriString());
    ...
    public final Task<Void> remove(final CacheKey key) {
        ...
        try {
            ...
            FLog.m1316w(TAG, e, "Failed to schedule disk-cache remove for %s", key.getUriString());
            Task<Void> taskForError = Task.forError(e);
            Intrinsics.checkNotNullExpressionValue(taskForError, "{\n      // Log failure\n …forError(exception)\n    }");
            return taskForError;
        }
    ...
    public final Task<Void> clearAll() {
        ...
        try {
            ...
            FLog.m1316w(TAG, e, "Failed to schedule disk-cache clear", new Object[0]);
            Task<Void> taskForError = Task.forError(e);
            Intrinsics.checkNotNullExpressionValue(taskForError, "{\n      // Log failure\n …forError(exception)\n    }");
            return taskForError;
        }
    ...
    private final PooledByteBuffer readFromDiskCache(CacheKey key) throws IOException {
        try {
            Class<?> cls = TAG;
            FLog.m1298v(cls, "Disk cache read for %s", key.getUriString());
            BinaryResource resource = this.fileCache.getResource(key);
            if (resource == null) {
                FLog.m1298v(cls, "Disk cache miss for %s", key.getUriString());
                this.imageCacheStatsTracker.onDiskCacheMiss(key);
                return null;
            }
            FLog.m1298v(cls, "Found entry in disk cache for %s", key.getUriString());
            ...
            try {
                ...
                FLog.m1298v(cls, "Successful read from disk cache for %s", key.getUriString());
            ...
            FLog.m1316w(TAG, e, "Exception reading from cache for %s", key.getUriString());
    ...
    private final void writeToDiskCache(CacheKey key, final EncodedImage encodedImage) {
        Class<?> cls = TAG;
        FLog.m1298v(cls, "About to write to disk-cache for key %s", key.getUriString());
        try {
            ...
            FLog.m1298v(cls, "Successful disk-cache write for key %s", key.getUriString());
        } catch (IOException e) {
            FLog.m1316w(TAG, e, "Failed to write to disk-cache for key %s", key.getUriString());
        }
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void writeToDiskCache$lambda$7(EncodedImage encodedImage, BufferedDiskCache this$0, OutputStream os2) throws IOException {
        Intrinsics.checkNotNullParameter(this$0, "this$0");
        ...
        this$0.pooledByteStreams.copy(inputStream, os2);
    }
}
