...
public class ImagePipelineFactory {
    ...
    public static synchronized void initialize(ImagePipelineConfigInterface imagePipelineConfigInterface) {
        if (sInstance != null) {
            FLog.m1313w(TAG, "ImagePipelineFactory has already been initialized! `ImagePipelineFactory.initialize(...)` should only be called once to avoid unexpected behavior.");
    ...
    public AnimatedCache getAnimatedCache(int i) {
        if (this.mAnimatedCache == null) {
            this.mAnimatedCache = AnimatedCache.getInstance((int) (((Math.min(Runtime.getRuntime().maxMemory(), 2147483647L) / 100) * ((long) i)) / PlaybackStateCompat.ACTION_SET_CAPTIONING_ENABLED));
        }
