...
public class DirectorySoSource extends SoSource {
    ...
    protected int loadLibraryFrom(String str, int i, File file, StrictMode.ThreadPolicy threadPolicy) throws IOException {
        ...
        if (this.denyList.contains(str)) {
            LogInstrumentation.m2726d("SoLoader", str + " is on the denyList, skip loading from " + file.getCanonicalPath());
            return 0;
        }
        File soFileByName = getSoFileByName(str);
        if (soFileByName == null) {
            LogInstrumentation.m2732v("SoLoader", str + " not found on " + file.getCanonicalPath());
            return 0;
        }
        LogInstrumentation.m2726d("SoLoader", str + " found on " + file.getCanonicalPath());
        if ((i & 1) != 0 && (this.flags & 2) != 0) {
            LogInstrumentation.m2726d("SoLoader", str + " loaded implicitly");
            return 2;
        }
        ...
        if (z) {
            ...
            LogInstrumentation.m2726d("SoLoader", "Not resolving dependencies for " + str);
        }
        try {
            if (zEquals) {
            ...
            if (e.getMessage().contains("bad ELF magic")) {
                LogInstrumentation.m2726d("SoLoader", "Corrupted lib file detected");
                if (channel == null) {
                    return 3;
                }
                channel.close();
    ...
    private void loadDependencies(String str, ElfByteChannel elfByteChannel, int i, StrictMode.ThreadPolicy threadPolicy) throws IOException {
        ...
        LogInstrumentation.m2726d("SoLoader", "Loading " + str + "'s dependencies: " + Arrays.toString(dependencies));
