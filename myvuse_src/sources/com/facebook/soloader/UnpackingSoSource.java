...
public abstract class UnpackingSoSource extends DirectorySoSource {
    ...
    public static void writeState(File file, byte b) throws IOException {
        try {
            ...
            LogInstrumentation.m2735w(TAG, "state file sync failed", e);
        }
    }
        ...
        for (String str : list) {
            if (!str.equals(STATE_FILE_NAME) && !str.equals(LOCK_FILE_NAME) && !str.equals(INSTANCE_LOCK_FILE_NAME) && !str.equals(DEPS_FILE_NAME) && !str.equals(MANIFEST_FILE_NAME)) {
                ...
                if (!z) {
                    ...
                    LogInstrumentation.m2732v(TAG, "deleting unaccounted-for file " + file);
    ...
    private void extractDso(InputDso inputDso, byte[] bArr) throws IOException {
        LogInstrumentation.m2730i(TAG, "extracting DSO " + inputDso.getDso().name);
        try {
            ...
            if (!this.soDirectory.setWritable(false)) {
                LogInstrumentation.m2734w(TAG, "error removing " + this.soDirectory.getCanonicalPath() + " write permission");
            }
        }
    }
        ...
        try {
            try {
                if (file.exists() && !file.setWritable(true)) {
                    LogInstrumentation.m2734w(TAG, "error adding write permission to: " + file);
                }
                try {
                    randomAccessFile = new RandomAccessFile(file, "rw");
                } catch (IOException e) {
                    LogInstrumentation.m2735w(TAG, "error overwriting " + file + " trying to delete and start over", e);
                    SysUtil.dumbDeleteRecursive(file);
                    randomAccessFile = new RandomAccessFile(file, "rw");
                }
                ...
                if (!file.setWritable(false)) {
                    LogInstrumentation.m2734w(TAG, "error removing " + file + " write permission");
                }
    ...
    private void regenerate(byte b, DsoManifest dsoManifest, InputDsoIterator inputDsoIterator) throws IOException {
        ...
        LogInstrumentation.m2732v(TAG, "regenerating DSO store " + getClass().getName());
        ...
        try {
            if (b == 1) {
                try {
                    ...
                    LogInstrumentation.m2731i(TAG, "error reading existing DSO manifest", e);
                    dsoManifest2 = null;
                }
            } else {
                dsoManifest2 = null;
            ...
            randomAccessFile.close();
            LogInstrumentation.m2732v(TAG, "Finished regenerating DSO store " + getClass().getName());
        } catch (Throwable th) {
            try {
                randomAccessFile.close();
            } catch (Throwable th2) {
    ...
    protected boolean refreshLocked(FileLocker fileLocker, int i, byte[] bArr) throws IOException {
        ...
        if (b != 1) {
            LogInstrumentation.m2732v(TAG, "dso store " + this.soDirectory + " regeneration interrupted: wiping clean");
            b = 0;
        }
        randomAccessFile.close();
        ...
        try {
            ...
            if (randomAccessFile.read(bArr2) != length) {
                LogInstrumentation.m2732v(TAG, "short read of so store deps file: marking unclean");
                b = 0;
            }
            if (depsChanged(bArr2, bArr)) {
                LogInstrumentation.m2732v(TAG, "deps mismatch on deps store: regenerating");
                b = 0;
            }
            if (b == 0 || (i & 2) != 0) {
                LogInstrumentation.m2732v(TAG, "so store dirty: regenerating");
    ...
    private Runnable createSyncer(final FileLocker fileLocker, final byte[] bArr, final File file, final File file2, final DsoManifest dsoManifest, final Boolean bool) {
        return new Runnable() { // from class: com.facebook.soloader.UnpackingSoSource.1
            ...
            public void run() {
                try {
                    try {
                        LogInstrumentation.m2732v(UnpackingSoSource.TAG, "starting syncer worker");
                    ...
                    LogInstrumentation.m2732v(UnpackingSoSource.TAG, "releasing dso store lock for " + UnpackingSoSource.this.soDirectory + " (from syncer thread)");
                    fileLocker.close();
                }
            }
        };
