...
public final class NativeDeps {
        ...
        new Thread(new Runnable() { // from class: com.facebook.soloader.NativeDeps.1
            ...
            public void run() {
                ...
                try {
                    ...
                    if (readLockCount >= 3) {
                        LogInstrumentation.m2734w(NativeDeps.LOG_TAG, "NativeDeps initialization finished with " + Integer.toString(readLockCount) + " threads waiting.");
                    }
    ...
    public static boolean useDepsFileFromApkSync(Context context, boolean z) throws Throwable {
        ...
        if (!zInitDeps) {
            LogInstrumentation.m2734w(LOG_TAG, "Failed to extract native deps from APK, falling back to using MinElf to get library dependencies.");
        }
        return zInitDeps;
    }
