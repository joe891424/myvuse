...
public abstract class BaseLayer implements DrawingContent, BaseKeyframeAnimation.AnimationListener, KeyPathElement {
    ...
    static BaseLayer forModel(CompositionLayer compositionLayer, Layer layer, LottieDrawable lottieDrawable, LottieComposition lottieComposition) {
        switch (C13821.$SwitchMap$com$airbnb$lottie$model$layer$Layer$LayerType[layer.getLayerType().ordinal()]) {
            case 1:
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        Paint paint;
        Integer value;
        C1363L.beginSection(this.drawTraceName);
        if (!this.visible || this.layerModel.isHidden()) {
            C1363L.endSection(this.drawTraceName);
            return;
        }
        buildParentLayerListIfNeeded();
        C1363L.beginSection("Layer#parentMatrix");
        this.matrix.reset();
        this.matrix.set(matrix);
        for (int size = this.parentLayers.size() - 1; size >= 0; size--) {
            this.matrix.preConcat(this.parentLayers.get(size).transform.getMatrix());
        }
        C1363L.endSection("Layer#parentMatrix");
        ...
        if (!hasMatteOnThisLayer() && !hasMasksOnThisLayer()) {
            ...
            C1363L.beginSection("Layer#drawLayer");
            drawLayer(canvas, this.matrix, iIntValue);
            C1363L.endSection("Layer#drawLayer");
            recordRenderTime(C1363L.endSection(this.drawTraceName));
            return;
        }
        C1363L.beginSection("Layer#computeBounds");
        ...
        C1363L.endSection("Layer#computeBounds");
        if (this.rect.width() >= 1.0f && this.rect.height() >= 1.0f) {
            C1363L.beginSection("Layer#saveLayer");
            this.contentPaint.setAlpha(255);
            Utils.saveLayerCompat(canvas, this.rect, this.contentPaint);
            C1363L.endSection("Layer#saveLayer");
            clearCanvas(canvas);
            C1363L.beginSection("Layer#drawLayer");
            drawLayer(canvas, this.matrix, iIntValue);
            C1363L.endSection("Layer#drawLayer");
            ...
            if (hasMatteOnThisLayer()) {
                C1363L.beginSection("Layer#drawMatte");
                C1363L.beginSection("Layer#saveLayer");
                Utils.saveLayerCompat(canvas, this.rect, this.mattePaint, 19);
                C1363L.endSection("Layer#saveLayer");
                clearCanvas(canvas);
                this.matteLayer.draw(canvas, matrix, iIntValue);
                C1363L.beginSection("Layer#restoreLayer");
                canvas.restore();
                C1363L.endSection("Layer#restoreLayer");
                C1363L.endSection("Layer#drawMatte");
            }
            C1363L.beginSection("Layer#restoreLayer");
            canvas.restore();
            C1363L.endSection("Layer#restoreLayer");
        ...
        if (hasMasksOnThisLayer()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (value != null) {
                    ...
                    int i2 = C13821.$SwitchMap$com$airbnb$lottie$model$content$Mask$MaskMode[mask.getMaskMode().ordinal()];
                    if (i2 == 1 || i2 == 2) {
                        return;
                    }
    ...
    void setProgress(float f) {
        C1363L.beginSection("BaseLayer#setProgress");
        C1363L.beginSection("BaseLayer#setProgress.transform");
        this.transform.setProgress(f);
        C1363L.endSection("BaseLayer#setProgress.transform");
        if (this.mask != null) {
            C1363L.beginSection("BaseLayer#setProgress.mask");
            for (int i = 0; i < this.mask.getMaskAnimations().size(); i++) {
                this.mask.getMaskAnimations().get(i).setProgress(f);
            }
            C1363L.endSection("BaseLayer#setProgress.mask");
        }
        if (this.inOutAnimation != null) {
            C1363L.beginSection("BaseLayer#setProgress.inout");
            this.inOutAnimation.setProgress(f);
            C1363L.endSection("BaseLayer#setProgress.inout");
        }
        if (this.matteLayer != null) {
            C1363L.beginSection("BaseLayer#setProgress.matte");
            this.matteLayer.setProgress(f);
            C1363L.endSection("BaseLayer#setProgress.matte");
        }
        C1363L.beginSection("BaseLayer#setProgress.animations." + this.animations.size());
        for (int i2 = 0; i2 < this.animations.size(); i2++) {
            this.animations.get(i2).setProgress(f);
        }
        C1363L.endSection("BaseLayer#setProgress.animations." + this.animations.size());
        C1363L.endSection("BaseLayer#setProgress");
    }
