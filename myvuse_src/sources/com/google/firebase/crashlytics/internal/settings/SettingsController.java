...
public class SettingsController implements SettingsProvider {
                ...
                if (cachedSettings != null) {
                    ...
                    if (settingsJson != null) {
                        ...
                        if (SettingsCacheBehavior.IGNORE_CACHE_EXPIRATION.equals(settingsCacheBehavior) || !settingsJson.isExpired(currentTimeMillis)) {
                            try {
                                Logger.getLogger().m2618v("Returning cached settings.");
                                settings = settingsJson;
                            } catch (Exception e) {
                                e = e;
                                settings = settingsJson;
                                Logger.getLogger().m2615e("Failed to get cached settings", e);
                            }
                        } else {
                            Logger.getLogger().m2618v("Cached settings have expired.");
                        }
                    } else {
                        Logger.getLogger().m2615e("Failed to parse cached settings data.", null);
                    }
                } else {
                    Logger.getLogger().m2612d("No cached settings data found.");
                }
            }
        } catch (Exception e2) {
            e = e2;
    ...
    public void logSettings(JSONObject jSONObject, String str) throws JSONException {
        Logger.getLogger().m2612d(str + (!(jSONObject instanceof JSONObject) ? jSONObject.toString() : JSONObjectInstrumentation.toString(jSONObject)));
    }
