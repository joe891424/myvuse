...
public class CrashlyticsCore {
    ...
    public boolean onPreExecute(AppData appData, SettingsProvider settingsProvider) {
        ...
        try {
            ...
            if (zDidPreviousInitializationFail && CommonUtils.canTryConnection(this.context)) {
                Logger.getLogger().m2612d("Crashlytics did not finish previous background initialization. Initializing synchronously.");
                finishInitSynchronously(settingsProvider);
                return false;
            }
            Logger.getLogger().m2612d("Successfully configured exception handler.");
            return true;
        } catch (Exception e) {
            Logger.getLogger().m2615e("Crashlytics was not started due to an exception during initialization", e);
    ...
    public Task<Void> doBackgroundInitialization(SettingsProvider settingsProvider) {
        ...
        try {
            ...
            if (!settingsProvider.getSettingsSync().featureFlagData.collectReports) {
                Logger.getLogger().m2612d("Collection of crash reports disabled in Crashlytics settings.");
                return Tasks.forException(new RuntimeException("Collection of crash reports disabled in Crashlytics settings."));
            }
            if (!this.controller.finalizeSessions(settingsProvider)) {
                Logger.getLogger().m2620w("Previous sessions could not be finalized.");
            }
            return this.controller.submitAllReports(settingsProvider.getSettingsAsync());
        } catch (Exception e) {
            Logger.getLogger().m2615e("Crashlytics encountered a problem during asynchronous initialization.", e);
    ...
    public void logFatalException(Throwable th) {
        Logger.getLogger().m2612d("Recorded on-demand fatal events: " + this.onDemandCounter.getRecordedOnDemandExceptions());
        Logger.getLogger().m2612d("Dropped on-demand fatal events: " + this.onDemandCounter.getDroppedOnDemandExceptions());
    ...
    private void finishInitSynchronously(final SettingsProvider settingsProvider) {
        ...
        Logger.getLogger().m2612d("Crashlytics detected incomplete initialization on previous app launch. Will initialize synchronously.");
        try {
            futureSubmit.get(3L, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            Logger.getLogger().m2615e("Crashlytics was interrupted during initialization.", e);
        } catch (ExecutionException e2) {
            Logger.getLogger().m2615e("Crashlytics encountered a problem during initialization.", e2);
        } catch (TimeoutException e3) {
            Logger.getLogger().m2615e("Crashlytics timed out during initialization.", e3);
        }
    }
    ...
    void markInitializationStarted() {
        ...
        Logger.getLogger().m2618v("Initialization marker file was created.");
    }
        ...
        this.backgroundWorker.submit(new Callable<Boolean>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsCore.3
            ...
            public Boolean call() throws Exception {
                try {
                    ...
                    if (!zRemove) {
                        Logger.getLogger().m2620w("Initialization marker file was not properly removed.");
                    }
                    return Boolean.valueOf(zRemove);
                } catch (Exception e) {
                    Logger.getLogger().m2615e("Problem encountered deleting Crashlytics initialization marker.", e);
                    return false;
                }
            }
        });
    ...
    static boolean isBuildIdValid(String str, boolean z) {
        if (!z) {
            Logger.getLogger().m2618v("Configured not to require a build ID.");
            return true;
        }
        if (!TextUtils.isEmpty(str)) {
            return true;
        }
        LogInstrumentation.m2728e(Logger.TAG, ".");
        LogInstrumentation.m2728e(Logger.TAG, ".     |  | ");
        LogInstrumentation.m2728e(Logger.TAG, ".     |  |");
        LogInstrumentation.m2728e(Logger.TAG, ".     |  |");
        LogInstrumentation.m2728e(Logger.TAG, ".   \\ |  | /");
        ...
        LogInstrumentation.m2728e(Logger.TAG, ".      \\/");
        LogInstrumentation.m2728e(Logger.TAG, ".");
        LogInstrumentation.m2728e(Logger.TAG, MISSING_BUILD_ID_MSG);
        LogInstrumentation.m2728e(Logger.TAG, ".");
        LogInstrumentation.m2728e(Logger.TAG, ".      /\\");
        LogInstrumentation.m2728e(Logger.TAG, ".     /  \\");
        LogInstrumentation.m2728e(Logger.TAG, ".    /    \\");
