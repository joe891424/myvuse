...
public class SessionReportingCoordinator implements CrashlyticsLifecycleEvents {
    ...
    public void persistFatalEvent(Throwable th, Thread thread, String str, long j) {
        Logger.getLogger().m2618v("Persisting fatal event for session " + str);
        persistEvent(th, thread, str, "crash", j, true);
    }

    public void persistNonFatalEvent(Throwable th, Thread thread, String str, long j) {
        Logger.getLogger().m2618v("Persisting non-fatal event for session " + str);
    ...
    public void persistRelevantAppExitInfoEvent(String str, List<ApplicationExitInfo> list, LogFileManager logFileManager, UserMetadata userMetadata) {
        ...
        if (applicationExitInfoFindRelevantApplicationExitInfo == null) {
            Logger.getLogger().m2618v("No relevant ApplicationExitInfo occurred during session: " + str);
            return;
        }
        ...
        Logger.getLogger().m2612d("Persisting anr for session " + str);
    ...
    public void finalizeSessionWithNativeEvent(String str, List<NativeSessionFile> list, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        Logger.getLogger().m2612d("SessionReportingCoordinator#finalizeSessionWithNativeEvent");
    ...
    private CrashlyticsReport.Session.Event addLogsAndCustomKeysToEvent(CrashlyticsReport.Session.Event event, LogFileManager logFileManager, UserMetadata userMetadata) {
        ...
        if (logString != null) {
            ...
            Logger.getLogger().m2618v("No log data to include with this event.");
        }
    ...
    public boolean onReportSendComplete(Task<CrashlyticsReportWithSessionId> task) {
        if (task.isSuccessful()) {
            ...
            Logger.getLogger().m2612d("Crashlytics report successfully enqueued to DataTransport: " + result.getSessionId());
            File reportFile = result.getReportFile();
            if (reportFile.delete()) {
                Logger.getLogger().m2612d("Deleted report file: " + reportFile.getPath());
                return true;
            }
            Logger.getLogger().m2620w("Crashlytics could not delete report file: " + reportFile.getPath());
            return true;
        }
        Logger.getLogger().m2621w("Crashlytics report could not be enqueued to DataTransport", task.getException());
        return false;
    }
    ...
    private static CrashlyticsReport.ApplicationExitInfo convertApplicationExitInfo(ApplicationExitInfo applicationExitInfo) {
        ...
        try {
            ...
            Logger.getLogger().m2620w("Could not get input trace in application exit info: " + applicationExitInfo.toString() + " Error: " + e);
        }
