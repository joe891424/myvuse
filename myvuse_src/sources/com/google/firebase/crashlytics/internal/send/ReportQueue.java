...
final class ReportQueue {
    ...
    TaskCompletionSource<CrashlyticsReportWithSessionId> enqueueReport(CrashlyticsReportWithSessionId crashlyticsReportWithSessionId, boolean z) {
        synchronized (this.queue) {
            ...
            if (z) {
                ...
                if (isQueueAvailable()) {
                    Logger.getLogger().m2612d("Enqueueing report: " + crashlyticsReportWithSessionId.getSessionId());
                    Logger.getLogger().m2612d("Queue size: " + this.queue.size());
                    this.singleThreadExecutor.execute(new ReportRunnable(crashlyticsReportWithSessionId, taskCompletionSource));
                    Logger.getLogger().m2612d("Closing task for report: " + crashlyticsReportWithSessionId.getSessionId());
                    taskCompletionSource.trySetResult(crashlyticsReportWithSessionId);
                    return taskCompletionSource;
                }
                calcStep();
                Logger.getLogger().m2612d("Dropping report due to queue being full: " + crashlyticsReportWithSessionId.getSessionId());
    ...
    public void flushScheduledReportsIfAble() {
        ...
        new Thread(new Runnable() { // from class: com.google.firebase.crashlytics.internal.send.ReportQueue$$ExternalSyntheticLambda0
            ...
            public final void run() {
                this.f$0.m2625x23ee29ee(countDownLatch);
            }
        }).start();
    ...
    /* synthetic */ void m2625x23ee29ee(CountDownLatch countDownLatch) {
        try {
    ...
    public void sendReport(final CrashlyticsReportWithSessionId crashlyticsReportWithSessionId, final TaskCompletionSource<CrashlyticsReportWithSessionId> taskCompletionSource) {
        Logger.getLogger().m2612d("Sending report through Google DataTransport: " + crashlyticsReportWithSessionId.getSessionId());
        final boolean z = SystemClock.elapsedRealtime() - this.startTimeMs < FaceCaptureConstants.DURATION_FACE_DETECT_SHIMMER_ANIMATION_MS;
        this.transport.schedule(Event.ofUrgent(crashlyticsReportWithSessionId.getReport()), new TransportScheduleCallback() { // from class: com.google.firebase.crashlytics.internal.send.ReportQueue$$ExternalSyntheticLambda1
            ...
            public final void onSchedule(Exception exc) {
                this.f$0.m2626xc033738(taskCompletionSource, z, crashlyticsReportWithSessionId, exc);
            }
        });
    }

    /* JADX INFO: renamed from: lambda$sendReport$1$com-google-firebase-crashlytics-internal-send-ReportQueue */
    /* synthetic */ void m2626xc033738(TaskCompletionSource taskCompletionSource, boolean z, CrashlyticsReportWithSessionId crashlyticsReportWithSessionId, Exception exc) {
        if (exc != null) {
    ...
    private final class ReportRunnable implements Runnable {
        ...
        public void run() {
            ...
            Logger.getLogger().m2612d("Delay for: " + String.format(Locale.US, "%.2f", Double.valueOf(dCalcDelay / 1000.0d)) + " s for report: " + this.reportWithSessionId.getSessionId());
