        ...
        for (int i = 0; i < rolloutMetadata.length(); i++) {
            try {
                ...
                if (jSONArray.length() > 1) {
                    LogInstrumentation.m2734w(FirebaseRemoteConfig.TAG, String.format("Rollout has multiple affected parameter keys.Only the first key will be included in RolloutsState. rolloutId: %s, affectedParameterKeys: %s", string, jSONArray));
                }
