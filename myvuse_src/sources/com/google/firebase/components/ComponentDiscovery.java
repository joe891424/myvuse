...
public final class ComponentDiscovery<T> {
    ...
    public List<ComponentRegistrar> discover() {
        ...
        while (it2.hasNext()) {
            try {
                ...
                LogInstrumentation.m2735w(TAG, "Invalid component registrar.", e);
            }
        }
        return arrayList;
    }
    ...
    public static ComponentRegistrar instantiate(String str) {
        try {
            ...
            LogInstrumentation.m2734w(TAG, String.format("Class %s is not an found.", str));
            return null;
    ...
    private static class MetadataRegistrarNameRetriever implements RegistrarNameRetriever<Context> {
        ...
        public List<String> retrieve(Context context) {
            ...
            if (metadata == null) {
                LogInstrumentation.m2734w(ComponentDiscovery.TAG, "Could not retrieve metadata, returning empty list of registrars.");
        ...
        private Bundle getMetadata(Context context) {
            try {
                ...
                if (packageManager == null) {
                    LogInstrumentation.m2734w(ComponentDiscovery.TAG, "Context has no PackageManager.");
                    return null;
                }
                ServiceInfo serviceInfo = packageManager.getServiceInfo(new ComponentName(context, this.discoveryService), 128);
                if (serviceInfo == null) {
                    LogInstrumentation.m2734w(ComponentDiscovery.TAG, this.discoveryService + " has no service info.");
                    return null;
                }
                return serviceInfo.metaData;
            } catch (PackageManager.NameNotFoundException unused) {
                LogInstrumentation.m2734w(ComponentDiscovery.TAG, "Application info not found.");
                return null;
            }
        }
    }
