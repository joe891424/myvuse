...
public final class SessionLifecycleService extends Service {
    ...
    /* JADX INFO: compiled from: SessionLifecycleService.kt */
    @Metadata(m5597d1 = {"\u0000B\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u000b\n\u0000\n\u0002\u0010\t\n\u0000\n\u0002\u0010\u0002\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0002\b\n\n\u0002\u0010\u000e\n\u0000\b\u0000\u0018\u00002\u00020\u0001B\r\u0012\u0006\u0010\u0002\u001a\u00020\u0003¢\u0006\u0002\u0010\u0004J\b\u0010\r\u001a\u00020\u000eH\u0002J\u0010\u0010\u000f\u001a\u00020\u000e2\u0006\u0010\u0010\u001a\u00020\u0011H\u0002J\u0010\u0010\u0012\u001a\u00020\u000e2\u0006\u0010\u0010\u001a\u00020\u0011H\u0002J\u0010\u0010\u0013\u001a\u00020\u000e2\u0006\u0010\u0010\u001a\u00020\u0011H\u0002J\u0010\u0010\u0014\u001a\u00020\u000e2\u0006\u0010\u0010\u001a\u00020\u0011H\u0016J\u0010\u0010\u0015\u001a\u00020\n2\u0006\u0010\u0016\u001a\u00020\fH\u0002J\u0010\u0010\u0017\u001a\u00020\u000e2\u0006\u0010\u0018\u001a\u00020\u0007H\u0002J\b\u0010\u0019\u001a\u00020\u000eH\u0002J\u0018\u0010\u001a\u001a\u00020\u000e2\u0006\u0010\u0018\u001a\u00020\u00072\u0006\u0010\u001b\u001a\u00020\u001cH\u0002R\u001e\u0010\u0005\u001a\u0012\u0012\u0004\u0012\u00020\u00070\u0006j\b\u0012\u0004\u0012\u00020\u0007`\bX\u0082\u0004¢\u0006\u0002\n\u0000R\u000e\u0010\t\u001a\u00020\nX\u0082\u000e¢\u0006\u0002\n\u0000R\u000e\u0010\u000b\u001a\u00020\fX\u0082\u000e¢\u0006\u0002\n\u0000¨\u0006\u001d"}, m5598d2 = {"Lcom/google/firebase/sessions/SessionLifecycleService$MessageHandler;", "Landroid/os/Handler;", "looper", "Landroid/os/Looper;", "(Landroid/os/Looper;)V", "boundClients", "Ljava/util/ArrayList;", "Landroid/os/Messenger;", "Lkotlin/collections/ArrayList;", "hasForegrounded", "", "lastMsgTimeMs", "", "broadcastSession", "", "handleBackgrounding", NotificationCompat.CATEGORY_MESSAGE, "Landroid/os/Message;", "handleClientBound", "handleForegrounding", "handleMessage", "isSessionRestart", "foregroundTimeMs", "maybeSendSessionToClient", "client", "newSession", "sendSessionToClient", "sessionId", "", "com.google.firebase-firebase-sessions"}, m5599k = 1, m5600mv = {1, 8, 0}, m5602xi = 48)
    public static final class MessageHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (this.lastMsgTimeMs > msg.getWhen()) {
                LogInstrumentation.m2726d(SessionLifecycleService.TAG, "Ignoring old message from " + msg.getWhen() + " which is older than " + this.lastMsgTimeMs + '.');
                return;
            }
            int i = msg.what;
            if (i == 1) {
            ...
            if (i == 2) {
                ...
                LogInstrumentation.m2734w(SessionLifecycleService.TAG, "Received unexpected event from the SessionLifecycleClient: " + msg);
                super.handleMessage(msg);
            }
        }

        private final void handleForegrounding(Message msg) {
            LogInstrumentation.m2726d(SessionLifecycleService.TAG, "Activity foregrounding at " + msg.getWhen() + '.');
            if (!this.hasForegrounded) {
                LogInstrumentation.m2726d(SessionLifecycleService.TAG, "Cold start detected.");
                this.hasForegrounded = true;
                newSession();
            } else if (isSessionRestart(msg.getWhen())) {
                LogInstrumentation.m2726d(SessionLifecycleService.TAG, "Session too long in background. Creating new session.");
                newSession();
            }
        ...
        private final void handleBackgrounding(Message msg) {
            LogInstrumentation.m2726d(SessionLifecycleService.TAG, "Activity backgrounding at " + msg.getWhen());
            this.lastMsgTimeMs = msg.getWhen();
        }
        ...
        private final void handleClientBound(Message msg) {
            ...
            LogInstrumentation.m2726d(SessionLifecycleService.TAG, "Client " + msg.replyTo + " bound at " + msg.getWhen() + ". Clients: " + this.boundClients.size());
        }

        private final void newSession() {
            SessionGenerator.INSTANCE.getInstance().generateNewSession();
            LogInstrumentation.m2726d(SessionLifecycleService.TAG, "Generated new session " + SessionGenerator.INSTANCE.getInstance().getCurrentSession().getSessionId());
            broadcastSession();
        ...
        private final void broadcastSession() {
            LogInstrumentation.m2726d(SessionLifecycleService.TAG, "Broadcasting new session: " + SessionGenerator.INSTANCE.getInstance().getCurrentSession());
        ...
        private final void maybeSendSessionToClient(Messenger client) {
            ...
            LogInstrumentation.m2726d(SessionLifecycleService.TAG, "App has not yet foregrounded. Using previously stored session: " + currentSessionId);
        ...
        private final void sendSessionToClient(Messenger client, String sessionId) {
            try {
                ...
                LogInstrumentation.m2726d(SessionLifecycleService.TAG, "Removing dead client from list: " + client);
                this.boundClients.remove(client);
            } catch (Exception e) {
                LogInstrumentation.m2735w(SessionLifecycleService.TAG, "Unable to push new session to " + client + '.', e);
            }
        }

        private final boolean isSessionRestart(long foregroundTimeMs) {
            return foregroundTimeMs - this.lastMsgTimeMs > Duration.m8577getInWholeMillisecondsimpl(SessionsSettings.INSTANCE.getInstance().m6317getSessionRestartTimeoutUwyO8pc());
        }
    }
    ...
    public IBinder onBind(Intent intent) {
        if (intent == null) {
            LogInstrumentation.m2726d(TAG, "Service bound with null intent. Ignoring.");
            return null;
        }
        LogInstrumentation.m2726d(TAG, "Service bound to new client on process " + intent.getAction());
