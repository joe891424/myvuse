...
class GmsRpc {
    ...
    private void setDefaultAttributesToBundle(String str, String str2, Bundle bundle) throws ExecutionException, InterruptedException {
        ...
        try {
            ...
            if (TextUtils.isEmpty(token)) {
                LogInstrumentation.m2734w("FirebaseMessaging", "FIS auth token is empty");
            } else {
                bundle.putString(PARAM_FIS_AUTH_TOKEN, token);
            }
        } catch (InterruptedException | ExecutionException e) {
            LogInstrumentation.m2729e("FirebaseMessaging", "Failed to get FIS auth token", e);
        }
    ...
    private String handleResponse(Bundle bundle) throws IOException {
        ...
        LogInstrumentation.m2735w("FirebaseMessaging", "Unexpected response: " + bundle, new Throwable());
        throw new IOException(ERROR_SERVICE_NOT_AVAILABLE);
    }
