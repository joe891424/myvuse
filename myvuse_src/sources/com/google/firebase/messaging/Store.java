...
class Store {
    ...
    private void checkForRestore(Context context, String str) {
        ...
        try {
            ...
            LogInstrumentation.m2730i(Constants.TAG, "App restored, clearing state");
            deleteAll();
        } catch (IOException e) {
            if (Log.isLoggable(Constants.TAG, 3)) {
                LogInstrumentation.m2726d(Constants.TAG, "Error creating file in no backup dir: " + e.getMessage());
            }
        }
    }
    ...
    static class Token {
        ...
        static Token parse(String str) {
            ...
            if (str.startsWith("{")) {
                try {
                    ...
                    LogInstrumentation.m2734w(Constants.TAG, "Failed to parse token: " + e);
                    return null;
                }
            }
        ...
        static String encode(String str, String str2, long j) {
            try {
                ...
                LogInstrumentation.m2734w(Constants.TAG, "Failed to encode token: " + e);
                return null;
            }
        }
