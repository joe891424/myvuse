...
class DisplayNotification {
    ...
    private void waitForAndApplyImageDownload(NotificationCompat.Builder builder, ImageDownload imageDownload) {
        ...
        try {
            ...
            LogInstrumentation.m2734w(Constants.TAG, "Interrupted while downloading image, showing notification without it");
            imageDownload.close();
            Thread.currentThread().interrupt();
        } catch (ExecutionException e) {
            LogInstrumentation.m2734w(Constants.TAG, "Failed to download image: " + e.getCause());
        } catch (TimeoutException unused2) {
            LogInstrumentation.m2734w(Constants.TAG, "Failed to download image in time, showing notification without it");
            imageDownload.close();
        }
    }
    ...
    private void showNotification(CommonNotificationBuilder.DisplayNotificationInfo displayNotificationInfo) {
        if (Log.isLoggable(Constants.TAG, 3)) {
            LogInstrumentation.m2726d(Constants.TAG, "Showing notification");
        }
        ((NotificationManager) this.context.getSystemService("notification")).notify(displayNotificationInfo.tag, displayNotificationInfo.f3848id, displayNotificationInfo.notificationBuilder.build());
    }
}
