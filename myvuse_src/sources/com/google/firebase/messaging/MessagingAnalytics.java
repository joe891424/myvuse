...
public class MessagingAnalytics {
    ...
    static boolean deliveryMetricsExportToBigQueryEnabled() {
        ...
        try {
            ...
            LogInstrumentation.m2730i(Constants.TAG, "FirebaseApp has not being initialized. Device might be in direct boot mode. Skip exporting delivery metrics to Big Query");
            return false;
        }
    }
    ...
    private static void setUserPropertyIfRequired(Bundle bundle) {
        ...
        if ("1".equals(bundle.getString(Constants.AnalyticsKeys.TRACK_CONVERSIONS))) {
            ...
            if (Log.isLoggable(Constants.TAG, 3)) {
                LogInstrumentation.m2726d(Constants.TAG, "Received event with track-conversion=true. Setting user property and reengagement event");
            }
            ...
            LogInstrumentation.m2734w(Constants.TAG, "Unable to set user property for conversion tracking:  analytics library is missing");
            return;
        }
        if (Log.isLoggable(Constants.TAG, 3)) {
            LogInstrumentation.m2726d(Constants.TAG, "Received event with track-conversion=false. Do not set user property");
        }
    }
        ...
        try {
            ...
            if (messageTime != null) {
                try {
                    ...
                    LogInstrumentation.m2735w(Constants.TAG, "Error while parsing timestamp in GCM event", e);
                }
            }
            ...
            if (useDeviceTime != null) {
                try {
                    ...
                    LogInstrumentation.m2735w(Constants.TAG, "Error while parsing use_device_time in GCM event", e2);
                }
            }
            ...
            if (Log.isLoggable(Constants.TAG, 3)) {
                LogInstrumentation.m2726d(Constants.TAG, "Logging to scion event=" + str + " scionPayload=" + bundle2);
            }
            AnalyticsConnector analyticsConnector = (AnalyticsConnector) FirebaseApp.getInstance().get(AnalyticsConnector.class);
            if (analyticsConnector == null) {
                LogInstrumentation.m2734w(Constants.TAG, "Unable to log event: analytics library is missing");
            } else {
                analyticsConnector.logEvent("fcm", str, bundle2);
            }
        } catch (IllegalStateException unused) {
            LogInstrumentation.m2728e(Constants.TAG, "Default FirebaseApp has not been initialized. Skip logging event to GA.");
        }
    }

    private static void logToFirelog(MessagingClientEvent.Event event, Intent intent, TransportFactory transportFactory) {
        if (transportFactory == null) {
            LogInstrumentation.m2728e(Constants.TAG, "TransportFactory is null. Skip exporting message delivery metrics to Big Query");
            return;
        }
        ...
        try {
            transportFactory.getTransport(Constants.FirelogAnalytics.FCM_LOG_SOURCE, MessagingClientEventExtension.class, Encoding.m2320of("proto"), new Transformer() { // from class: com.google.firebase.messaging.MessagingAnalytics$$ExternalSyntheticLambda0
            ...
            LogInstrumentation.m2735w(Constants.TAG, "Failed to send big query analytics payload.", e);
        }
    }
    ...
    static int getTtl(Bundle bundle) {
        ...
        try {
            ...
            LogInstrumentation.m2734w(Constants.TAG, "Invalid TTL: " + obj);
            return 0;
        }
    }
    ...
    static String getInstanceId(Bundle bundle) {
        String string = bundle.getString(Constants.MessagePayloadKeys.f3849TO);
    ...
    static long getProjectNumber(Bundle bundle) {
        if (bundle.containsKey(Constants.MessagePayloadKeys.SENDER_ID)) {
            try {
                ...
                LogInstrumentation.m2735w(Constants.TAG, "error parsing project number", e);
            }
        }
        ...
        if (gcmSenderId != null) {
            try {
                ...
                LogInstrumentation.m2735w(Constants.TAG, "error parsing sender ID", e2);
            }
        }
        ...
        if (!applicationId.startsWith("1:")) {
            try {
                ...
                LogInstrumentation.m2735w(Constants.TAG, "error parsing app ID", e3);
            }
        } else {
            ...
            try {
                ...
                LogInstrumentation.m2735w(Constants.TAG, "error parsing app ID", e4);
            }
        }
        return 0L;
    }
