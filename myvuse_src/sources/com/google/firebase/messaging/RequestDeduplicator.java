...
class RequestDeduplicator {
    ...
    synchronized Task<String> getOrStartGetTokenRequest(final String str, GetTokenRequest getTokenRequest) {
        ...
        if (task != null) {
            if (Log.isLoggable(Constants.TAG, 3)) {
                LogInstrumentation.m2726d(Constants.TAG, "Joining ongoing request for: " + str);
            }
            return task;
        }
        if (Log.isLoggable(Constants.TAG, 3)) {
            LogInstrumentation.m2726d(Constants.TAG, "Making new request for: " + str);
        }
        Task taskContinueWithTask = getTokenRequest.start().continueWithTask(this.executor, new Continuation() { // from class: com.google.firebase.messaging.RequestDeduplicator$$ExternalSyntheticLambda0
            ...
            public final Object then(Task task2) {
                return this.f$0.m2646x7161fc54(str, task2);
            }
        });
    ...
    /* synthetic */ Task m2646x7161fc54(String str, Task task) throws Exception {
        synchronized (this) {
