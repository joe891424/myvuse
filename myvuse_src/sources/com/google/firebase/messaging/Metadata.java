...
class Metadata {
    ...
    synchronized int getIidImplementation() {
        ...
        if (packageManager.checkPermission(GMSCORE_SEND_PERMISSION, "com.google.android.gms") == -1) {
            LogInstrumentation.m2728e(Constants.TAG, "Google Play services missing or without correct permission.");
            return 0;
        }
        ...
        LogInstrumentation.m2734w(Constants.TAG, "Failed to resolve IID implementation package, falling back");
        if (PlatformVersion.isAtLeastO()) {
            this.iidImplementation = 2;
        } else {
            this.iidImplementation = 1;
    ...
    private PackageInfo getPackageInfo(String str) {
        try {
            ...
            LogInstrumentation.m2734w(Constants.TAG, "Failed to find package " + e);
            return null;
        }
    }
}
