...
public class ServiceStarter {
    ...
    public int startMessagingService(Context context, Intent intent) {
        if (Log.isLoggable(Constants.TAG, 3)) {
            LogInstrumentation.m2726d(Constants.TAG, "Starting service");
        }
    ...
    private int doStartService(Context context, Intent intent) {
        ...
        if (strResolveServiceClassName != null) {
            if (Log.isLoggable(Constants.TAG, 3)) {
                LogInstrumentation.m2726d(Constants.TAG, "Restricting intent to a specific service: " + strResolveServiceClassName);
            }
        ...
        try {
            if (hasWakeLockPermission(context)) {
                ...
                LogInstrumentation.m2726d(Constants.TAG, "Missing wake lock permission, service start may be delayed");
            }
            if (componentNameStartService != null) {
                return -1;
            }
            LogInstrumentation.m2728e(Constants.TAG, "Error while delivering the message: ServiceIntent not found.");
            return ERROR_NOT_FOUND;
        } catch (IllegalStateException e) {
            LogInstrumentation.m2728e(Constants.TAG, "Failed to start service while in background: " + e);
            return 402;
        } catch (SecurityException e2) {
            LogInstrumentation.m2729e(Constants.TAG, "Error while delivering the message to the serviceIntent", e2);
            return 401;
        }
    }
    ...
    private synchronized String resolveServiceClassName(Context context, Intent intent) {
        ...
        if (resolveInfoResolveService != null && resolveInfoResolveService.serviceInfo != null) {
            ...
            LogInstrumentation.m2728e(Constants.TAG, "Error resolving target intent service, skipping classname enforcement. Resolved service was: " + serviceInfo.packageName + "/" + serviceInfo.name);
            return null;
        }
        LogInstrumentation.m2728e(Constants.TAG, "Failed to resolve target intent service, skipping classname enforcement");
        return null;
    }
    ...
    boolean hasWakeLockPermission(Context context) {
        ...
        if (!this.hasWakeLockPermission.booleanValue() && Log.isLoggable(Constants.TAG, 3)) {
            LogInstrumentation.m2726d(Constants.TAG, "Missing Permission: android.permission.WAKE_LOCK this should normally be included by the manifest merger, but may needed to be manually added to your manifest");
        }
    ...
    boolean hasAccessNetworkStatePermission(Context context) {
        ...
        if (!this.hasWakeLockPermission.booleanValue() && Log.isLoggable(Constants.TAG, 3)) {
            LogInstrumentation.m2726d(Constants.TAG, "Missing Permission: android.permission.ACCESS_NETWORK_STATE this should normally be included by the manifest merger, but may needed to be manually added to your manifest");
        }
