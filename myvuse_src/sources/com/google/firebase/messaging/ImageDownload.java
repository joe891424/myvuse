...
public class ImageDownload implements Closeable {
    ...
    public static ImageDownload create(String str) {
        ...
        try {
            ...
            LogInstrumentation.m2734w(Constants.TAG, "Not downloading image, bad URL: " + str);
            return null;
        }
    }
    ...
    public void start(ExecutorService executorService) {
        ...
        this.future = executorService.submit(new Runnable() { // from class: com.google.firebase.messaging.ImageDownload$$ExternalSyntheticLambda0
            ...
            public final void run() {
                this.f$0.m6305lambda$start$0$comgooglefirebasemessagingImageDownload(taskCompletionSource);
            }
        });
    ...
    /* synthetic */ void m6305lambda$start$0$comgooglefirebasemessagingImageDownload(TaskCompletionSource taskCompletionSource) {
        try {
    ...
    public Bitmap blockingDownload() throws IOException {
        if (Log.isLoggable(Constants.TAG, 4)) {
            LogInstrumentation.m2730i(Constants.TAG, "Starting download of: " + this.url);
        }
        byte[] bArrBlockingDownloadBytes = blockingDownloadBytes();
        Bitmap bitmapDecodeByteArray = BitmapFactoryInstrumentation.decodeByteArray(bArrBlockingDownloadBytes, 0, bArrBlockingDownloadBytes.length);
        if (bitmapDecodeByteArray == null) {
            throw new IOException("Failed to decode image: " + this.url);
        }
        if (Log.isLoggable(Constants.TAG, 3)) {
            LogInstrumentation.m2726d(Constants.TAG, "Successfully downloaded image: " + this.url);
        }
        return bitmapDecodeByteArray;
    }

    private byte[] blockingDownloadBytes() throws IOException {
        URLConnection uRLConnectionOpenConnection = URLConnectionInstrumentation.openConnection(this.url.openConnection());
        ...
        try {
            ...
            if (Log.isLoggable(Constants.TAG, 2)) {
                LogInstrumentation.m2732v(Constants.TAG, "Downloaded " + byteArray.length + " bytes from " + this.url);
            }
