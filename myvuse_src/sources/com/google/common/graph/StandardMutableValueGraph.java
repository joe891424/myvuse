...
final class StandardMutableValueGraph<N, V> extends StandardValueGraph<N, V> implements MutableValueGraph<N, V> {
    ...
    public boolean removeNode(N n) {
        ...
        while (it2.hasNext()) {
            ((GraphConnections) Objects.requireNonNull(this.nodeConnections.getWithoutCaching(it2.next()))).removePredecessor(n);
            this.edgeCount--;
        }
        if (isDirected()) {
            Iterator<N> it3 = graphConnections.predecessors().iterator();
            while (it3.hasNext()) {
                Preconditions.checkState(((GraphConnections) Objects.requireNonNull(this.nodeConnections.getWithoutCaching(it3.next()))).removeSuccessor(n) != null);
                this.edgeCount--;
            }
        }
    ...
    private GraphConnections<N, V> newConnections() {
        if (isDirected()) {
            return DirectedGraphConnections.m2545of(this.incidentEdgeOrder);
        }
        return UndirectedGraphConnections.m2551of(this.incidentEdgeOrder);
    }
}
