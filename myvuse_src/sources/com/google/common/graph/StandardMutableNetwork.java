...
final class StandardMutableNetwork<N, E> extends StandardNetwork<N, E> implements MutableNetwork<N, E> {
    ...
    public boolean addEdge(N n, N n2, E e) {
        ...
        if (containsEdge(e)) {
            ...
            EndpointPair endpointPairM2549of = EndpointPair.m2549of(this, n, n2);
            Preconditions.checkArgument(endpointPairIncidentNodes.equals(endpointPairM2549of), "Edge %s already exists between the following nodes: %s, so it cannot be reused to connect the following nodes: %s.", e, endpointPairIncidentNodes, endpointPairM2549of);
            return false;
        }
    ...
    private NetworkConnections<N, E> newConnections() {
        if (isDirected()) {
            if (allowsParallelEdges()) {
                return DirectedMultiNetworkConnections.m2546of();
            }
            return DirectedNetworkConnections.m2547of();
        }
        if (allowsParallelEdges()) {
            return UndirectedMultiNetworkConnections.m2552of();
        }
        return UndirectedNetworkConnections.m2553of();
    }
}
