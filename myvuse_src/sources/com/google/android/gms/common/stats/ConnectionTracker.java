...
public class ConnectionTracker {
    ...
    private final boolean zzc(Context context, String str, Intent intent, ServiceConnection serviceConnection, int i, boolean z, @Nullable Executor executor) {
        ...
        if (component != null) {
            ...
            try {
                if ((Wrappers.packageManager(context).getApplicationInfo(packageName, 0).flags & 2097152) != 0) {
                    LogInstrumentation.m2734w("ConnectionTracker", "Attempted to bind to a service in a STOPPED package.");
                    return false;
                }
        ...
        if (serviceConnection2 != null && serviceConnection != serviceConnection2) {
            LogInstrumentation.m2734w("ConnectionTracker", String.format("Duplicate binding with the same ServiceConnection: %s, %s, %s.", serviceConnection, str, intent.getAction()));
        }
        try {
