...
final class zzal<T> extends zzae<T> {
    ...
    protected final T zza(SharedPreferences sharedPreferences) {
        try {
            ...
            LogInstrumentation.m2729e("PhenotypeFlag", strValueOf.length() != 0 ? "Invalid byte[] value in SharedPreferences for ".concat(strValueOf) : new String("Invalid byte[] value in SharedPreferences for "), e);
            return null;
        }
    }
    ...
    protected final T zzb(String str) {
        ...
        try {
            ...
            LogInstrumentation.m2728e("PhenotypeFlag", new StringBuilder(String.valueOf(str2).length() + 27 + String.valueOf(str).length()).append("Invalid byte[] value for ").append(str2).append(": ").append(str).toString());
            return null;
        }
    }
}
