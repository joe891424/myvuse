...
public class BitmapInStreamingChecker {
    ...
    public void check(InputImage inputImage) {
        ...
        if (this.zzb.size() != 5 || jElapsedRealtime - ((Long) Preconditions.checkNotNull(this.zzb.peekFirst())).longValue() >= 5000) {
            return;
        }
        long j = this.zzc;
        if (j == -1 || jElapsedRealtime - j >= TimeUnit.SECONDS.toMillis(5L)) {
            this.zzc = jElapsedRealtime;
            zza.m2365w("StreamingFormatChecker", "ML Kit has detected that you seem to pass camera frames to the detector as a Bitmap object. This is inefficient. Please use YUV_420_888 format for camera2 API or NV21 format for (legacy) camera API and directly pass down the byte array to ML Kit.");
        }
    }
}
