...
public final class zza implements RemoteModelFileMover {
    ...
    private static boolean zza(File file, File file2) {
        ...
        if (file.renameTo(file2)) {
            zza.m2357d("CustomModelFileMover", String.format("Moved file from %s to %s successfully", absolutePath, absolutePath2));
            file2.setExecutable(false);
            file2.setWritable(false);
            return true;
        }
        GmsLogger gmsLogger = zza;
        gmsLogger.m2357d("CustomModelFileMover", String.format("Move file to %s failed, remove the temp file %s.", absolutePath2, absolutePath));
        if (!file.delete()) {
            String strValueOf = String.valueOf(absolutePath);
            gmsLogger.m2357d("CustomModelFileMover", strValueOf.length() != 0 ? "Failed to delete the temp file: ".concat(strValueOf) : new String("Failed to delete the temp file: "));
        }
        return false;
    }
