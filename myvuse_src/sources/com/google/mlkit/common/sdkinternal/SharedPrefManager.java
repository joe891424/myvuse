...
public class SharedPrefManager {
    ...
    static final /* synthetic */ SharedPrefManager zzc(ComponentContainer componentContainer) {
    ...
    public synchronized void clearIncompatibleModelInfo(RemoteModel remoteModel) {
        zzd().edit().remove(String.format("bad_hash_%s", remoteModel.getUniqueModelNameForPersist())).remove(C4352k.a.f5663q).apply();
    }
    ...
    public synchronized Long getDownloadingModelId(RemoteModel remoteModel) {
        long j = zzd().getLong(String.format("downloading_model_id_%s", remoteModel.getUniqueModelNameForPersist()), -1L);
        if (j < 0) {
            return null;
        }
    ...
    public synchronized String getMlSdkInstanceId() {
        ...
        zzd().edit().putString("ml_sdk_instance_id", string2).apply();
        return string2;
    }
    ...
    public synchronized String getPreviousAppVersion() {
        return zzd().getString(C4352k.a.f5663q, null);
    }
    ...
    public synchronized void setDownloadingModelInfo(long j, ModelInfo modelInfo) {
        ...
        zzd().edit().putString(String.format("downloading_model_hash_%s", modelNameForPersist), modelInfo.getModelHash()).putLong(String.format("downloading_model_id_%s", modelNameForPersist), j).putLong(String.format("downloading_begin_time_%s", modelNameForPersist), SystemClock.elapsedRealtime()).apply();
    }

    public synchronized void setIncompatibleModelInfo(RemoteModel remoteModel, String str, String str2) {
        zzd().edit().putString(String.format("bad_hash_%s", remoteModel.getUniqueModelNameForPersist()), str).putString(C4352k.a.f5663q, str2).apply();
    }
    ...
    public synchronized void setModelFirstUseTimeMs(RemoteModel remoteModel, long j) {
        zzd().edit().putLong(String.format("model_first_use_time_%s", remoteModel.getUniqueModelNameForPersist()), j).apply();
    }

    public final synchronized String zza(String str, long j) {
        return zzd().getString(String.format("cached_local_model_hash_%1s_%2s", Preconditions.checkNotNull(str), Long.valueOf(j)), null);
    }

    public final synchronized void zzb(String str, long j, String str2) {
        zzd().edit().putString(String.format("cached_local_model_hash_%1s_%2s", Preconditions.checkNotNull(str), Long.valueOf(j)), str2).apply();
    }
}
