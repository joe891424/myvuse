...
class KmlStyleParser {
    ...
    private static void setIconHotSpot(XmlPullParser xmlPullParser, KmlStyle kmlStyle) {
        try {
            kmlStyle.setHotSpot(Float.parseFloat(xmlPullParser.getAttributeValue(null, EllipticCurveJsonWebKey.X_MEMBER_NAME)), Float.parseFloat(xmlPullParser.getAttributeValue(null, EllipticCurveJsonWebKey.Y_MEMBER_NAME)), xmlPullParser.getAttributeValue(null, "xunits"), xmlPullParser.getAttributeValue(null, "yunits"));
        } catch (NullPointerException unused) {
            LogInstrumentation.m2734w(LOG_TAG, "Missing 'x' or 'y' attributes in hotSpot for style with id: " + kmlStyle.getStyleId());
        } catch (NumberFormatException unused2) {
            LogInstrumentation.m2734w(LOG_TAG, "Invalid number in 'x' or 'y' attributes in hotSpot for style with id: " + kmlStyle.getStyleId());
        }
    }
