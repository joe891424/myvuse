...
public class ReactDatabaseSupplier extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sQLiteDatabase) {
        if (sQLiteDatabase instanceof SQLiteDatabase) {
            SQLiteInstrumentation.execSQL(sQLiteDatabase, VERSION_TABLE_CREATE);
        } else {
    ...
    public synchronized void clearAndCloseDatabase() throws RuntimeException {
        try {
            clear();
            closeDatabase();
            FLog.m1265d(ReactConstants.TAG, "Cleaned RKStorage");
        } catch (Exception unused) {
            if (deleteDatabase()) {
                FLog.m1265d(ReactConstants.TAG, "Deleted Local Database RKStorage");
                return;
            }
    ...
    synchronized void clear() {
        ...
        if (sQLiteDatabase instanceof SQLiteDatabase) {
            SQLiteInstrumentation.delete(sQLiteDatabase, TABLE_CATALYST, null, null);
        } else {
            sQLiteDatabase.delete(TABLE_CATALYST, null, null);
        }
    }
