...
public class RNCWebViewClient extends WebViewClient {
    ...
    public boolean shouldOverrideUrlLoading(WebView webView, String str) {
        ...
        if (rNCWebView.getReactApplicationContext().getJavaScriptContextHolder().get() != 0 && rNCWebView.mMessagingJSModule != null) {
            ...
            try {
                synchronized (atomicReference) {
                    ...
                    while (atomicReference.get() == RNCWebViewModuleImpl.ShouldOverrideUrlLoadingLock.ShouldOverrideCallbackState.UNDECIDED) {
                        if (SystemClock.elapsedRealtime() - jElapsedRealtime > 250) {
                            FLog.m1317w(TAG, "Did not receive response to shouldOverrideUrlLoading in time, defaulting to allow loading.");
                ...
                FLog.m1278e(TAG, "shouldOverrideUrlLoading was interrupted while waiting for result.", e);
                RNCWebViewModuleImpl.shouldOverrideUrlLoadingLock.removeLock(Double.valueOf(dDoubleValue));
                return false;
            }
        }
        FLog.m1317w(TAG, "Couldn't use blocking synchronous call for onShouldStartLoadWithRequest due to debugging or missing Catalyst instance, falling back to old event-and-load.");
    ...
    public void onReceivedSslError(WebView webView, SslErrorHandler sslErrorHandler, SslError sslError) {
        ...
        if (!url.equalsIgnoreCase(url2)) {
            LogInstrumentation.m2734w(TAG, "Resource blocked from loading due to SSL error. Blocked URL: " + url2);
            return;
        }
    ...
    public void onReceivedHttpError(WebView webView, WebResourceRequest webResourceRequest, WebResourceResponse webResourceResponse) {
        ...
        if (webResourceRequest.isForMainFrame()) {
            ...
            writableMapCreateWebViewEvent.putInt(AnalyticsAttribute.STATUS_CODE_ATTRIBUTE, webResourceResponse.getStatusCode());
    ...
    public boolean onRenderProcessGone(WebView webView, RenderProcessGoneDetail renderProcessGoneDetail) {
        ...
        if (renderProcessGoneDetail.didCrash()) {
            LogInstrumentation.m2728e(TAG, "The WebView rendering process crashed.");
        } else {
            LogInstrumentation.m2734w(TAG, "The WebView rendering process was killed by the system.");
        }
        if (webView == null) {
            return true;
        }
