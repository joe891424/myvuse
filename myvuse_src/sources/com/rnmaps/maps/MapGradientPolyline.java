...
public class MapGradientPolyline extends MapFeature {
    ...
    public class AirMapGradientPolylineProvider implements TileProvider {
        ...
        public void renderTrail(Canvas canvas, Matrix matrix, Paint paint, Paint paint2, float f, int i, int i2) {
            ...
            if (this.points.size() == 1) {
                ...
                canvas2.drawCircle((float) mutPoint.f4215x, (float) mutPoint.f4216y, paint2.getStrokeWidth() / 2.0f, paint2);
                paint2.setStyle(Paint.Style.STROKE);
                return;
            }
            ...
            while (i3 < this.points.size()) {
                ...
                LogInstrumentation.m2726d("AirMapGradientPolyline", String.valueOf(f4));
                paint2.setStyle(Paint.Style.FILL);
                paint2.setColor(MapGradientPolyline.interpolateColor(this.colors, f4));
                canvas2.drawCircle((float) mutPoint2.f4215x, (float) mutPoint2.f4216y, paint2.getStrokeWidth() / 2.0f, paint2);
                paint2.setStyle(Paint.Style.STROKE);
        ...
        public void drawLine(Canvas canvas, Matrix matrix, Paint paint, Paint paint2, MutPoint mutPoint, MutPoint mutPoint2, float f, float f2) {
            ...
            matrix.reset();
            matrix.preRotate((float) Math.toDegrees(Math.atan2(mutPoint2.f4216y - mutPoint.f4216y, mutPoint2.f4215x - mutPoint.f4215x)), (float) mutPoint.f4215x, (float) mutPoint.f4216y);
            matrix.preTranslate((float) mutPoint.f4215x, (float) mutPoint.f4216y);
            float fSqrt = (float) Math.sqrt(Math.pow(mutPoint2.f4215x - mutPoint.f4215x, 2.0d) + Math.pow(mutPoint2.f4216y - mutPoint.f4216y, 2.0d));
            matrix.preScale(fSqrt, fSqrt);
            float f3 = 1.0f / (f2 - f);
            matrix.preScale(f3, f3);
            matrix.preTranslate(-f, 0.0f);
            paint.getShader().setLocalMatrix(matrix);
            canvas.drawLine((float) mutPoint.f4215x, (float) mutPoint.f4216y, (float) mutPoint2.f4215x, (float) mutPoint2.f4216y, paint);
        }
    ...
    public static class MutPoint {
        ...
        public MutPoint set(Point point, float f, int i, int i2, int i3) {
            double d = f;
            this.f4215x = (point.f3881x * d) - ((double) (i * i3));
            this.f4216y = (point.f3882y * d) - ((double) (i2 * i3));
            return this;
        }
    }
}
