...
public class GifHeaderParser {
    ...
    private void readBitmap() {
        this.header.currentFrame.f2051ix = readShort();
        this.header.currentFrame.f2052iy = readShort();
        this.header.currentFrame.f2050iw = readShort();
        this.header.currentFrame.f2049ih = readShort();
        int i = read();
        boolean z = (i & 128) != 0;
    ...
    private int[] readColorTable(int i) {
        ...
        try {
            ...
            if (Log.isLoggable(TAG, 3)) {
                LogInstrumentation.m2727d(TAG, "Format Error Reading Color Table", e);
            }
            this.header.status = 1;
        }
        return iArr;
    ...
    private void readBlock() {
        ...
        while (true) {
            try {
                ...
                if (Log.isLoggable(TAG, 3)) {
                    LogInstrumentation.m2727d(TAG, "Error Reading Block n: " + i2 + " count: " + i3 + " blockSize: " + this.blockSize, e);
                }
                this.header.status = 1;
                return;
            }
