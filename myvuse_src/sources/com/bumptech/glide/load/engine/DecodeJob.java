...
class DecodeJob<R> implements DataFetcherGenerator.FetcherReadyCallback, Runnable, Comparable<DecodeJob<?>>, FactoryPools.Poolable {
        ...
        try {
            try {
                ...
                if (Log.isLoggable(TAG, 3)) {
                    LogInstrumentation.m2727d(TAG, "DecodeJob threw unexpectedly, isCancelled: " + this.isCancelled + ", stage: " + this.stage, th);
                }
    ...
    private void runWrapped() {
        int i = C16101.$SwitchMap$com$bumptech$glide$load$engine$DecodeJob$RunReason[this.runReason.ordinal()];
        if (i == 1) {
    ...
    private DataFetcherGenerator getNextGenerator() {
        int i = C16101.$SwitchMap$com$bumptech$glide$load$engine$DecodeJob$Stage[this.stage.ordinal()];
        if (i == 1) {
    ...
    private Stage getNextStage(Stage stage) {
        int i = C16101.$SwitchMap$com$bumptech$glide$load$engine$DecodeJob$Stage[stage.ordinal()];
        if (i == 1) {
    ...
    private void logWithTimeAndKey(String str, long j, String str2) {
        LogInstrumentation.m2732v(TAG, str + " in " + LogTime.getElapsedMillis(j) + ", load key: " + this.loadKey + (str2 != null ? ", " + str2 : "") + ", thread: " + Thread.currentThread().getName());
    }
    ...
    <Z> Resource<Z> onResourceDecoded(DataSource dataSource, Resource<Z> resource) {
        ...
        int i = C16101.$SwitchMap$com$bumptech$glide$load$EncodeStrategy[encodeStrategy.ordinal()];
        if (i == 1) {
