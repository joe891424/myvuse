...
public final class LruArrayPool implements ArrayPool {
    ...
    private <T> T getForKey(Key key, Class<T> cls) {
        ...
        if (Log.isLoggable(adapterFromType.getTag(), 2)) {
            LogInstrumentation.m2732v(adapterFromType.getTag(), "Allocated " + key.size + " bytes");
        }
    ...
    private void evictToSize(int i) {
        while (this.currentSize > i) {
            ...
            if (Log.isLoggable(adapterFromObject.getTag(), 2)) {
                LogInstrumentation.m2732v(adapterFromObject.getTag(), "evicted: " + adapterFromObject.getArrayLength(objRemoveLast));
            }
        }
    }
