...
public class LruBitmapPool implements BitmapPool {
        ...
        try {
            ...
            if (bitmap.isMutable() && this.strategy.getSize(bitmap) <= this.maxSize && this.allowedConfigs.contains(bitmap.getConfig())) {
                ...
                if (Log.isLoggable(TAG, 2)) {
                    LogInstrumentation.m2732v(TAG, "Put bitmap in pool=" + this.strategy.logBitmap(bitmap));
                }
                dump();
                evict();
                return;
            ...
            if (Log.isLoggable(TAG, 2)) {
                LogInstrumentation.m2732v(TAG, "Reject bitmap from pool, bitmap: " + this.strategy.logBitmap(bitmap) + ", is mutable: " + bitmap.isMutable() + ", is allowed config: " + this.allowedConfigs.contains(bitmap.getConfig()));
            }
            bitmap.recycle();
        } catch (Throwable th) {
            throw th;
    ...
    private synchronized Bitmap getDirtyOrNull(int i, int i2, Bitmap.Config config) {
        ...
        if (bitmap == null) {
            if (Log.isLoggable(TAG, 3)) {
                LogInstrumentation.m2726d(TAG, "Missing bitmap=" + this.strategy.logBitmap(i, i2, config));
            }
            this.misses++;
        } else {
            this.hits++;
        ...
        if (Log.isLoggable(TAG, 2)) {
            LogInstrumentation.m2732v(TAG, "Get bitmap=" + this.strategy.logBitmap(i, i2, config));
        }
        dump();
        return bitmap;
    }
    ...
    public void clearMemory() {
        if (Log.isLoggable(TAG, 3)) {
            LogInstrumentation.m2726d(TAG, "clearMemory");
        }
        trimToSize(0L);
    }
    ...
    public void trimMemory(int i) {
        if (Log.isLoggable(TAG, 3)) {
            LogInstrumentation.m2726d(TAG, "trimMemory, level=" + i);
        }
        if (i >= 40 || i >= 20) {
            clearMemory();
        } else if (i >= 20 || i == 15) {
        ...
        while (this.currentSize > j) {
            ...
            if (bitmapRemoveLast == null) {
                if (Log.isLoggable(TAG, 5)) {
                    LogInstrumentation.m2734w(TAG, "Size mismatch, resetting");
                    dumpUnchecked();
                }
                this.currentSize = 0L;
                return;
            ...
            this.evictions++;
            if (Log.isLoggable(TAG, 3)) {
                LogInstrumentation.m2726d(TAG, "Evicting bitmap=" + this.strategy.logBitmap(bitmapRemoveLast));
            }
            dump();
    ...
    private void dumpUnchecked() {
        LogInstrumentation.m2732v(TAG, "Hits=" + this.hits + ", misses=" + this.misses + ", puts=" + this.puts + ", evictions=" + this.evictions + ", currentSize=" + this.currentSize + ", maxSize=" + this.maxSize + "\nStrategy=" + this.strategy);
    }
