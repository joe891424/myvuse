...
class EngineJob<R> implements DecodeJob.Callback<R>, FactoryPools.Poolable {
    ...
    void notifyCallbacksOfResult() {
        synchronized (this) {
            ...
            for (ResourceCallbackAndExecutor resourceCallbackAndExecutor : resourceCallbacksAndExecutorsCopy) {
                resourceCallbackAndExecutor.executor.execute(new CallResourceReady(resourceCallbackAndExecutor.f2058cb));
            }
    ...
    void notifyCallbacksOfException() {
        synchronized (this) {
            ...
            for (ResourceCallbackAndExecutor resourceCallbackAndExecutor : resourceCallbacksAndExecutorsCopy) {
                resourceCallbackAndExecutor.executor.execute(new CallLoadFailed(resourceCallbackAndExecutor.f2058cb));
            }
            decrementPendingCallbacks();
        }
    }
    ...
    private class CallLoadFailed implements Runnable {
        ...
        private final ResourceCallback f2056cb;

        CallLoadFailed(ResourceCallback resourceCallback) {
            this.f2056cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f2056cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f2056cb)) {
                        EngineJob.this.callCallbackOnLoadFailed(this.f2056cb);
                    }
    ...
    private class CallResourceReady implements Runnable {
        ...
        /* JADX INFO: renamed from: cb */
        private final ResourceCallback f2057cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f2057cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f2057cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f2057cb)) {
                        EngineJob.this.engineResource.acquire();
                        EngineJob.this.callCallbackOnResourceReady(this.f2057cb);
                        EngineJob.this.removeCallback(this.f2057cb);
                    }
                    EngineJob.this.decrementPendingCallbacks();
                }
            }
    ...
    static final class ResourceCallbackAndExecutor {
        ...
        /* JADX INFO: renamed from: cb */
        final ResourceCallback f2058cb;
        final Executor executor;

        ResourceCallbackAndExecutor(ResourceCallback resourceCallback, Executor executor) {
            this.f2058cb = resourceCallback;
            this.executor = executor;
        }

        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f2058cb.equals(((ResourceCallbackAndExecutor) obj).f2058cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f2058cb.hashCode();
        }
    }
