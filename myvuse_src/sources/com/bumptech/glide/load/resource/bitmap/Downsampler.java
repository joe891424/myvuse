...
public final class Downsampler {
        ...
        if (shouldUsePool(imageType)) {
            if (i7 < 0 || i8 < 0 || !z2) {
                ...
                if (Log.isLoggable(TAG, 2)) {
                    LogInstrumentation.m2732v(TAG, "Calculated target [" + iRound + EllipticCurveJsonWebKey.X_MEMBER_NAME + iRound2 + "] for source [" + i7 + EllipticCurveJsonWebKey.X_MEMBER_NAME + i8 + "], sampleSize: " + i10 + ", targetDensity: " + options.inTargetDensity + ", density: " + options.inDensity + ", density multiplier: " + f);
                }
            } else {
                iRound = i4;
                iRound2 = i5;
    ...
    private static void calculateScaling(ImageHeaderParser.ImageType imageType, ImageReader imageReader, DecodeCallbacks decodeCallbacks, BitmapPool bitmapPool, DownsampleStrategy downsampleStrategy, int i, int i2, int i3, int i4, int i5, BitmapFactory.Options options) throws IOException {
        ...
        if (i2 <= 0 || i3 <= 0) {
            if (Log.isLoggable(TAG, 3)) {
                LogInstrumentation.m2726d(TAG, "Unable to determine dimensions for: " + imageType + " with target [" + i4 + EllipticCurveJsonWebKey.X_MEMBER_NAME + i5 + "]");
                return;
            }
            return;
        }
        ...
        if (scaleFactor <= 0.0f) {
            throw new IllegalArgumentException("Cannot scale with factor: " + scaleFactor + " from: " + downsampleStrategy + ", source: [" + i2 + EllipticCurveJsonWebKey.X_MEMBER_NAME + i3 + "], target: [" + i4 + EllipticCurveJsonWebKey.X_MEMBER_NAME + i5 + "]");
        }
        ...
        if (Log.isLoggable(TAG, 2)) {
            LogInstrumentation.m2732v(TAG, "Calculate scaling, source: [" + i2 + EllipticCurveJsonWebKey.X_MEMBER_NAME + i3 + "], degreesToRotate: " + i + ", target: [" + i4 + EllipticCurveJsonWebKey.X_MEMBER_NAME + i5 + "], power of two scaled: [" + iFloor + EllipticCurveJsonWebKey.X_MEMBER_NAME + iFloor2 + "], exact scale factor: " + scaleFactor + ", power of 2 sample size: " + iMax + ", adjusted scale factor: " + scaleFactor2 + ", target density: " + options.inTargetDensity + ", density: " + options.inDensity);
        }
    }
    ...
    private void calculateConfig(ImageReader imageReader, DecodeFormat decodeFormat, boolean z, boolean z2, BitmapFactory.Options options, int i, int i2) {
        ...
        try {
            ...
            if (Log.isLoggable(TAG, 3)) {
                LogInstrumentation.m2727d(TAG, "Cannot determine whether the image has alpha or not from header, format " + decodeFormat, e);
            }
            zHasAlpha = false;
        }
    ...
    */
    private static android.graphics.Bitmap decodeStream(com.bumptech.glide.load.resource.bitmap.ImageReader r5, android.graphics.BitmapFactory.Options r6, com.bumptech.glide.load.resource.bitmap.Downsampler.DecodeCallbacks r7, com.bumptech.glide.load.engine.bitmap_recycle.BitmapPool r8) throws java.io.IOException {
        ...
        L27:
            ...
            com.newrelic.agent.android.instrumentation.LogInstrumentation.m2727d(r0, r2, r1)     // Catch: java.lang.Throwable -> L25
        L38:
    ...
    private static void logDecode(int i, int i2, String str, BitmapFactory.Options options, Bitmap bitmap, int i3, int i4, long j) {
        LogInstrumentation.m2732v(TAG, "Decoded " + getBitmapString(bitmap) + " from [" + i + EllipticCurveJsonWebKey.X_MEMBER_NAME + i2 + "] " + str + " with inBitmap " + getInBitmapString(options) + " for [" + i3 + EllipticCurveJsonWebKey.X_MEMBER_NAME + i4 + "], sample size: " + options.inSampleSize + ", density: " + options.inDensity + ", target density: " + options.inTargetDensity + ", thread: " + Thread.currentThread().getName() + ", duration: " + LogTime.getElapsedMillis(j));
    }
    ...
    private static String getBitmapString(Bitmap bitmap) {
        ...
        return "[" + bitmap.getWidth() + EllipticCurveJsonWebKey.X_MEMBER_NAME + bitmap.getHeight() + "] " + bitmap.getConfig() + (" (" + bitmap.getAllocationByteCount() + ")");
    }
