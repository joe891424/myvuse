    ...
    public boolean isHardwareConfigAllowed(int i, int i2, boolean z, boolean z2) {
        if (!z) {
            if (Log.isLoggable(TAG, 2)) {
                LogInstrumentation.m2732v(TAG, "Hardware config disallowed by caller");
            }
            return false;
        }
        if (!this.isHardwareConfigAllowedByDeviceModel) {
            if (Log.isLoggable(TAG, 2)) {
                LogInstrumentation.m2732v(TAG, "Hardware config disallowed by device model");
            }
            return false;
        }
        if (!HARDWARE_BITMAPS_SUPPORTED) {
            if (Log.isLoggable(TAG, 2)) {
                LogInstrumentation.m2732v(TAG, "Hardware config disallowed by sdk");
            }
            return false;
        }
        if (areHardwareBitmapsBlockedByAppState()) {
            if (Log.isLoggable(TAG, 2)) {
                LogInstrumentation.m2732v(TAG, "Hardware config disallowed by app state");
            }
            return false;
        }
        if (z2) {
            if (Log.isLoggable(TAG, 2)) {
                LogInstrumentation.m2732v(TAG, "Hardware config disallowed because exif orientation is required");
            }
            return false;
        }
        ...
        if (i < i3) {
            if (Log.isLoggable(TAG, 2)) {
                LogInstrumentation.m2732v(TAG, "Hardware config disallowed because width is too small");
            }
            return false;
        }
        if (i2 < i3) {
            if (Log.isLoggable(TAG, 2)) {
                LogInstrumentation.m2732v(TAG, "Hardware config disallowed because height is too small");
            }
            return false;
        }
        ...
        if (Log.isLoggable(TAG, 2)) {
            LogInstrumentation.m2732v(TAG, "Hardware config disallowed because there are insufficient FDs");
        }
        return false;
    }
    ...
    private synchronized boolean isFdSizeBelowHardwareLimit() {
        ...
        if (i >= 50) {
            ...
            if (!z && Log.isLoggable("Downsampler", 5)) {
                LogInstrumentation.m2734w("Downsampler", "Excluding HARDWARE bitmap config because we're over the file descriptor limit, file descriptors " + length + ", limit " + maxFdCount);
            }
        }
