...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
        ...
        try {
            ...
            if (!handles(uInt16)) {
                if (Log.isLoggable(TAG, 3)) {
                    LogInstrumentation.m2726d(TAG, "Parser doesn't handle magic number: " + uInt16);
                }
                return -1;
            }
            ...
            if (iMoveToExifSegmentAndGetLength == -1) {
                if (Log.isLoggable(TAG, 3)) {
                    LogInstrumentation.m2726d(TAG, "Failed to parse exif segment length, or exif segment not found");
                }
                return -1;
            }
    ...
    private int parseExifSegment(Reader reader, byte[] bArr, int i) throws IOException {
        ...
        if (i2 != i) {
            if (Log.isLoggable(TAG, 3)) {
                LogInstrumentation.m2726d(TAG, "Unable to read exif segment data, length: " + i + ", actually read: " + i2);
            }
            return -1;
        }
        ...
        if (Log.isLoggable(TAG, 3)) {
            LogInstrumentation.m2726d(TAG, "Missing jpeg exif preamble");
        }
        return -1;
    }
    ...
    private int moveToExifSegmentAndGetLength(Reader reader) throws IOException {
        ...
        do {
            ...
            if (uInt82 != 255) {
                if (Log.isLoggable(TAG, 3)) {
                    LogInstrumentation.m2726d(TAG, "Unknown segmentId=" + ((int) uInt82));
                }
                return -1;
            }
            ...
            if (uInt8 == 217) {
                if (Log.isLoggable(TAG, 3)) {
                    LogInstrumentation.m2726d(TAG, "Found MARKER_EOI in exif segment");
                }
                return -1;
            }
        ...
        if (Log.isLoggable(TAG, 3)) {
            LogInstrumentation.m2726d(TAG, "Unable to skip enough data, type: " + ((int) uInt8) + ", wanted to skip: " + uInt16 + ", but actually skipped: " + jSkip);
        }
        return -1;
    }
    ...
    private static int parseExifSegment(RandomAccessReader randomAccessReader) {
        ...
        if (int16 == INTEL_TIFF_MAGIC_NUMBER) {
            ...
            if (int16 != MOTOROLA_TIFF_MAGIC_NUMBER && Log.isLoggable(TAG, 3)) {
                LogInstrumentation.m2726d(TAG, "Unknown endianness = " + ((int) int16));
            }
        ...
        for (int i = 0; i < int162; i++) {
            ...
            if (int163 == 274) {
                ...
                if (int164 < 1 || int164 > 12) {
                    if (Log.isLoggable(TAG, 3)) {
                        LogInstrumentation.m2726d(TAG, "Got invalid format code = " + ((int) int164));
                    }
                } else {
                    int int322 = randomAccessReader.getInt32(iCalcTagOffset + 4);
                    if (int322 < 0) {
                        if (Log.isLoggable(TAG, 3)) {
                            LogInstrumentation.m2726d(TAG, "Negative tiff component count");
                        }
                    } else {
                        if (Log.isLoggable(TAG, 3)) {
                            LogInstrumentation.m2726d(TAG, "Got tagIndex=" + i + " tagType=" + ((int) int163) + " formatCode=" + ((int) int164) + " componentCount=" + int322);
                        }
                        int i2 = int322 + BYTES_PER_FORMAT[int164];
                        if (i2 > 4) {
                            if (Log.isLoggable(TAG, 3)) {
                                LogInstrumentation.m2726d(TAG, "Got byte count > 4, not orientation, continuing, formatCode=" + ((int) int164));
                            }
                        } else {
                            int i3 = iCalcTagOffset + 8;
                            if (i3 < 0 || i3 > randomAccessReader.length()) {
                                if (Log.isLoggable(TAG, 3)) {
                                    LogInstrumentation.m2726d(TAG, "Illegal tagValueOffset=" + i3 + " tagType=" + ((int) int163));
                                }
                            } else if (i2 < 0 || i2 + i3 > randomAccessReader.length()) {
                                if (Log.isLoggable(TAG, 3)) {
                                    LogInstrumentation.m2726d(TAG, "Illegal number of bytes for TI tag data tagType=" + ((int) int163));
                                }
                            } else {
    ...
    private static final class StreamReader implements Reader {
        ...
        private final InputStream f2064is;

        StreamReader(InputStream inputStream) {
            this.f2064is = inputStream;
        }

        @Override // com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser.Reader
        public short getUInt8() throws IOException {
            int i = this.f2064is.read();
            if (i != -1) {
                return (short) i;
            }
            throw new Reader.EndOfFileException();
        ...
        public int read(byte[] bArr, int i) throws IOException {
            int i2 = 0;
            int i3 = 0;
            while (i2 < i && (i3 = this.f2064is.read(bArr, i2, i - i2)) != -1) {
                i2 += i3;
            }
            if (i2 == 0 && i3 == -1) {
                throw new Reader.EndOfFileException();
        ...
        public long skip(long j) throws IOException {
            ...
            long j2 = j;
            while (j2 > 0) {
                long jSkip = this.f2064is.skip(j2);
                if (jSkip <= 0) {
                    if (this.f2064is.read() == -1) {
                        break;
                    }
                    jSkip = 1;
                }
