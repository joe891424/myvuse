...
public class ByteBufferGifDecoder implements ResourceDecoder<ByteBuffer, GifDrawable> {
        ...
        try {
            ...
            if (header.getNumFrames() > 0 && header.getStatus() == 0) {
                ...
                if (Log.isLoggable(TAG, 2)) {
                    LogInstrumentation.m2732v(TAG, "Decoded GIF from stream in " + LogTime.getElapsedMillis(logTime));
                }
    ...
    private static int getSampleSize(GifHeader gifHeader, int i, int i2) {
        ...
        if (Log.isLoggable(TAG, 2) && iMax > 1) {
            LogInstrumentation.m2732v(TAG, "Downsampling GIF, sampleSize: " + iMax + ", target dimens: [" + i + EllipticCurveJsonWebKey.X_MEMBER_NAME + i2 + "], actual dimens: [" + gifHeader.getWidth() + EllipticCurveJsonWebKey.X_MEMBER_NAME + gifHeader.getHeight() + "]");
        }
        return iMax;
    }
