...
public class HttpUrlFetcher implements DataFetcher<InputStream> {
        ...
        try {
            try {
                ...
                if (Log.isLoggable(TAG, 3)) {
                    LogInstrumentation.m2727d(TAG, "Failed to load data for url", e);
                }
            ...
            if (Log.isLoggable(TAG, 2)) {
                sb = new StringBuilder("Finished http url fetcher fetch in ");
                LogInstrumentation.m2732v(TAG, sb.append(LogTime.getElapsedMillis(logTime)).toString());
            }
        } catch (Throwable th) {
            if (Log.isLoggable(TAG, 2)) {
                LogInstrumentation.m2732v(TAG, "Finished http url fetcher fetch in " + LogTime.getElapsedMillis(logTime));
            }
            throw th;
        }
    }
    ...
    private static int getHttpStatusCodeOrInvalid(HttpURLConnection httpURLConnection) {
        try {
            ...
            LogInstrumentation.m2727d(TAG, "Failed to get a response code", e);
            return -1;
        }
    }
    ...
    private InputStream getStreamForSuccessfulRequest(HttpURLConnection httpURLConnection) throws HttpException {
        try {
            if (TextUtils.isEmpty(httpURLConnection.getContentEncoding())) {
                ...
                if (Log.isLoggable(TAG, 3)) {
                    LogInstrumentation.m2726d(TAG, "Got non empty content encoding: " + httpURLConnection.getContentEncoding());
                }
