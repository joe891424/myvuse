...
public final class Util {
    ...
    public static int getBitmapByteSize(Bitmap bitmap) {
        if (bitmap.isRecycled()) {
            throw new IllegalStateException("Cannot obtain size for recycled Bitmap: " + bitmap + "[" + bitmap.getWidth() + EllipticCurveJsonWebKey.X_MEMBER_NAME + bitmap.getHeight() + "] " + bitmap.getConfig());
        }
        try {
    ...
    private static int getBytesPerPixel(Bitmap.Config config) {
        ...
        int i = C16541.$SwitchMap$android$graphics$Bitmap$Config[config.ordinal()];
        if (i == 1) {
            return 1;
        }
        if (i == 2 || i == 3) {
