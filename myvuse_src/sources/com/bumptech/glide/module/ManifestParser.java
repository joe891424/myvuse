...
public final class ManifestParser {
    ...
    public List<GlideModule> parse() {
        if (Log.isLoggable(TAG, 3)) {
            LogInstrumentation.m2726d(TAG, "Loading Glide modules");
        }
        ...
        try {
            ...
            if (applicationInfo.metaData == null) {
                if (Log.isLoggable(TAG, 3)) {
                    LogInstrumentation.m2726d(TAG, "Got null app info metadata");
                }
                return arrayList;
            }
            if (Log.isLoggable(TAG, 2)) {
                LogInstrumentation.m2732v(TAG, "Got app info metadata: " + applicationInfo.metaData);
            }
            for (String str : applicationInfo.metaData.keySet()) {
                if (GLIDE_MODULE_VALUE.equals(applicationInfo.metaData.get(str))) {
                    arrayList.add(parseModule(str));
                    if (Log.isLoggable(TAG, 3)) {
                        LogInstrumentation.m2726d(TAG, "Loaded Glide module: " + str);
                    }
                }
            }
            if (Log.isLoggable(TAG, 3)) {
                LogInstrumentation.m2726d(TAG, "Finished loading Glide modules");
            }
            return arrayList;
