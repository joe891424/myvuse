    ...
    public void onAttach(Context context) {
        ...
        if (rootFragmentManager == null) {
            if (Log.isLoggable(TAG, 5)) {
                LogInstrumentation.m2734w(TAG, "Unable to register fragment with root, ancestor detached");
            }
        } else {
            try {
                ...
                if (Log.isLoggable(TAG, 5)) {
                    LogInstrumentation.m2735w(TAG, "Unable to register fragment with root", e);
                }
            }
        }
    }
