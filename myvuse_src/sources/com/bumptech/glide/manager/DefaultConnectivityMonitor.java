...
final class DefaultConnectivityMonitor implements ConnectivityMonitor {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (z != DefaultConnectivityMonitor.this.isConnected) {
                if (Log.isLoggable(DefaultConnectivityMonitor.TAG, 3)) {
                    LogInstrumentation.m2726d(DefaultConnectivityMonitor.TAG, "connectivity changed, isConnected: " + DefaultConnectivityMonitor.this.isConnected);
                }
    ...
    private void register() {
        ...
        try {
            ...
            if (Log.isLoggable(TAG, 5)) {
                LogInstrumentation.m2735w(TAG, "Failed to register", e);
            }
        }
    }
    ...
    boolean isConnected(Context context) {
        try {
            ...
            if (Log.isLoggable(TAG, 5)) {
                LogInstrumentation.m2735w(TAG, "Failed to determine connectivity status when connectivity changed", e);
            }
            return true;
        }
    }
