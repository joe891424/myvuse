...
public class Glide implements ComponentCallbacks2 {
    ...
    public static File getPhotoCacheDir(Context context, String str) {
        ...
        if (Log.isLoggable(TAG, 6)) {
            LogInstrumentation.m2728e(TAG, "default disk cache dir is null");
        }
        return null;
    }
    ...
    private static void initializeGlide(Context context, GlideBuilder glideBuilder, GeneratedAppGlideModule generatedAppGlideModule) {
            ...
            while (it2.hasNext()) {
                ...
                if (excludedModuleClasses.contains(next.getClass())) {
                    if (Log.isLoggable(TAG, 3)) {
                        LogInstrumentation.m2726d(TAG, "AppGlideModule excludes manifest GlideModule: " + next);
                    }
                    it2.remove();
                }
            }
        ...
        if (Log.isLoggable(TAG, 3)) {
            ...
            while (it3.hasNext()) {
                LogInstrumentation.m2726d(TAG, "Discovered GlideModule from manifest: " + it3.next().getClass());
            }
        }
    ...
    private static GeneratedAppGlideModule getAnnotationGeneratedGlideModules(Context context) {
        try {
            ...
            if (Log.isLoggable(TAG, 5)) {
                LogInstrumentation.m2734w(TAG, "Failed to find GeneratedAppGlideModule. You should include an annotationProcessor compile dependency on com.github.bumptech.glide:compiler in your application and a @GlideModule annotated AppGlideModule implementation or LibraryGlideModules will be silently ignored");
            }
            return null;
