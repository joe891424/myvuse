...
class PathParser {
    ...
    private static int f3987i;

    /* JADX INFO: renamed from: l */
    private static int f3988l;
    ...
    static Path parse(String str) {
        ...
        while (f3987i < f3988l) {
            ...
            if (i < f3988l) {
                boolean z = true;
                boolean z2 = c != ' ';
                char cCharAt = f3989s.charAt(i);
                if (!z2 && cCharAt != 'M' && cCharAt != 'm') {
                    throw new Error(String.format("Unexpected character '%c' (i=%d, s=%s)", Character.valueOf(cCharAt), Integer.valueOf(f3987i), f3989s));
                }
                if (is_cmd(cCharAt)) {
                    f3987i++;
                    z = false;
                    ...
                    if (c == 'Z' || c == 'z') {
                        throw new Error(String.format("Unexpected number after 'z' (s=%s)", f3989s));
                    }
                    if (c == 'M' || c == 'm') {
                        c = is_absolute(c) ? 'L' : 'l';
                    } else {
                ...
                switch (c) {
                    ...
                    case 'v':
                        ...
                        break;
                    default:
                        throw new Error(String.format("Unexpected comand '%c' (s=%s)", Character.valueOf(c), f3989s));
                }
                if (z) {
                    c = zIs_absolute ? 'M' : 'm';
                }
    ...
    */
    ...
    private static void skip_spaces() {
        while (true) {
            int i = f3987i;
            if (i >= f3988l || !Character.isWhitespace(f3989s.charAt(i))) {
                return;
            } else {
                f3987i++;
            }
    ...
    private static boolean parse_flag() {
        skip_spaces();
        char cCharAt = f3989s.charAt(f3987i);
        if (cCharAt == '0' || cCharAt == '1') {
            int i = f3987i + 1;
            f3987i = i;
            if (i < f3988l && f3989s.charAt(i) == ',') {
                f3987i++;
            }
            skip_spaces();
            return cCharAt == '1';
        }
        throw new Error(String.format("Unexpected flag '%c' (i=%d, s=%s)", Character.valueOf(cCharAt), Integer.valueOf(f3987i), f3989s));
    }

    private static float parse_list_number() {
        if (f3987i == f3988l) {
            throw new Error(String.format("Unexpected end (s=%s)", f3989s));
        }
    ...
    private static float parse_number() {
        ...
        char cCharAt2 = f3989s.charAt(i);
        if (cCharAt2 == '-' || cCharAt2 == '+') {
            int i2 = f3987i + 1;
            f3987i = i2;
            cCharAt2 = f3989s.charAt(i2);
        }
        if (cCharAt2 >= '0' && cCharAt2 <= '9') {
            skip_digits();
            int i3 = f3987i;
            if (i3 < f3988l) {
                cCharAt2 = f3989s.charAt(i3);
            }
        } else if (cCharAt2 != '.') {
            throw new Error(String.format("Invalid number formating character '%c' (i=%d, s=%s)", Character.valueOf(cCharAt2), Integer.valueOf(f3987i), f3989s));
        }
        if (cCharAt2 == '.') {
            f3987i++;
            skip_digits();
            int i4 = f3987i;
            if (i4 < f3988l) {
                cCharAt2 = f3989s.charAt(i4);
            }
        }
        if (cCharAt2 == 'e' || cCharAt2 == 'E') {
            int i5 = f3987i;
            if (i5 + 1 < f3988l && (cCharAt = f3989s.charAt(i5 + 1)) != 'm' && cCharAt != 'x') {
                int i6 = f3987i + 1;
                f3987i = i6;
                char cCharAt3 = f3989s.charAt(i6);
                if (cCharAt3 == '+' || cCharAt3 == '-') {
                    ...
                    throw new Error(String.format("Invalid number formating character '%c' (i=%d, s=%s)", Character.valueOf(cCharAt3), Integer.valueOf(f3987i), f3989s));
                }
            }
        }
        String strSubstring = f3989s.substring(i, f3987i);
        float f = Float.parseFloat(strSubstring);
        if (Float.isInfinite(f) || Float.isNaN(f)) {
            throw new Error(String.format("Invalid number '%s' (start=%d, i=%d, s=%s)", strSubstring, Integer.valueOf(i), Integer.valueOf(f3987i), f3989s));
        }
        return f;
    }
    ...
    private static void skip_digits() {
        while (true) {
            int i = f3987i;
            if (i >= f3988l || !Character.isDigit(f3989s.charAt(i))) {
                return;
            } else {
                f3987i++;
            }
