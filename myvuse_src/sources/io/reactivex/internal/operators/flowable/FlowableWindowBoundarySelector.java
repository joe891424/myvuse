...
public final class FlowableWindowBoundarySelector<T, B, V> extends AbstractFlowableWithUpstream<T, Flowable<T>> {
    ...
    static final class WindowBoundaryMainSubscriber<T, B, V> extends QueueDrainSubscriber<T, Object, Flowable<T>> implements Subscription {
        ...
        final List<UnicastProcessor<T>> f8254ws;
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.upstream, subscription)) {
                ...
                if (PreviewView$1$$ExternalSyntheticBackportWithForwarding0.m1161m(this.boundary, null, operatorWindowBoundaryOpenSubscriber)) {
        ...
        public void onNext(T t) {
            ...
            if (fastEnter()) {
                Iterator<UnicastProcessor<T>> it2 = this.f8254ws.iterator();
                while (it2.hasNext()) {
                    it2.next().onNext(t);
                }
                if (leave(-1) == 0) {
        ...
        void drainLoop() {
            ...
            List<UnicastProcessor<T>> list = this.f8254ws;
            int iLeave = 1;
            while (true) {
                boolean z = this.done;
                ...
                if (!z2) {
                    if (objPoll instanceof WindowOperation) {
                        ...
                        if (windowOperation.f8255w != null) {
                            if (list.remove(windowOperation.f8255w)) {
                                windowOperation.f8255w.onComplete();
                                if (this.windows.decrementAndGet() == 0) {
                                    dispose();
                                    return;
                                }
        ...
        void close(OperatorWindowBoundaryCloseSubscriber<T, V> operatorWindowBoundaryCloseSubscriber) {
            ...
            this.queue.offer((U) new WindowOperation(operatorWindowBoundaryCloseSubscriber.f8253w, null));
            if (enter()) {
                drainLoop();
            }
        }
    ...
    static final class WindowOperation<T, B> {
        ...
        final UnicastProcessor<T> f8255w;

        WindowOperation(UnicastProcessor<T> unicastProcessor, B b) {
            this.f8255w = unicastProcessor;
            this.open = b;
        }
    }
    ...
    static final class OperatorWindowBoundaryCloseSubscriber<T, V> extends DisposableSubscriber<V> {
        ...
        /* JADX INFO: renamed from: w */
        final UnicastProcessor<T> f8253w;
