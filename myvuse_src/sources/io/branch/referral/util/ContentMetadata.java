...
public class ContentMetadata implements Parcelable {
    ...
    public JSONObject convertToJson() {
        ...
        try {
            ...
            if (this.imageCaptions.size() > 0) {
                ...
                Iterator<String> it2 = this.imageCaptions.iterator();
                while (it2.hasNext()) {
    ...
    public static ContentMetadata createFromJson(BranchUtil.JsonReader jsonReader) {
        ...
        JSONArray outJsonArray = jsonReader.readOutJsonArray(Defines.Jsonkey.ImageCaptions.getKey());
        if (outJsonArray != null) {
            for (int i = 0; i < outJsonArray.length(); i++) {
                contentMetadata.imageCaptions.add(outJsonArray.optString(i));
            }
        }
        try {
