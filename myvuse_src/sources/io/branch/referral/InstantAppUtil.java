...
class InstantAppUtil {
    ...
    static boolean doShowInstallPrompt(Activity activity, int i, String str) {
        if (activity == null) {
            BranchLogger.m5578v("Unable to show install prompt. Activity is null");
            return false;
        }
        if (!isInstantApp(activity)) {
            BranchLogger.m5578v("Unable to show install prompt. Application is not an instant app");
            return false;
        }
        ...
        if (!TextUtils.isEmpty(str)) {
            builderAppendQueryParameter.appendQueryParameter("referrer", str);
        }
